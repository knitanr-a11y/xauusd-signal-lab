...
DEFAULT_OUT_SUMMARY = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_mixed_sl_spread_summary.csv"
DEFAULT_OUT_TRADES = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_mixed_sl_spread_trades.csv"
DEFAULT_OUT_RULES = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_mixed_sl_spread_rules.md"
...
def spread_price(row: pd.Series, *, point_size: float, spread_multiplier: float) -> float:
    ...
    return max(0.0, spread_points * point_size * spread_multiplier)
...
def swing_stop(df: pd.DataFrame, *, signal_idx: int, entry_idx: int, side: str, lookback: int, atr_buffer: float, min_risk_atr: float, max_risk_atr: float, point_size: float, spread_multiplier: float) -> tuple[float, float, str, float] | None:
    ...
    entry = entry_mid + spread / 2 if side == "BUY" else entry_mid - spread / 2
    ...
    if side == "BUY":
        ...
        sl = anchor - buffer
        risk = entry - sl
        anchor_type = "recent_swing_low"
    else:
        anchor = float(recent["high"].max())
        sl = anchor + buffer
        risk = sl - entry
        anchor_type = "recent_swing_high"
    ...
    if risk_atr < min_risk_atr or risk_atr > max_risk_atr:
        return None
    return sl, risk, anchor_type, entry


def fixed_atr_stop(df: pd.DataFrame, *, signal_idx: int, entry_idx: int, side: str, risk_atr: float, point_size: float, spread_multiplier: float) -> tuple[float, float, str, float] | None:
    entry_row = df.iloc[entry_idx]
    entry_mid = float(entry_row["open"])
    spread = spread_price(entry_row, point_size=point_size, spread_multiplier=spread_multiplier)
    entry = entry_mid + spread / 2 if side == "BUY" else entry_mid - spread / 2
    ...
    if side == "BUY":
        ...
        sl = entry + risk
    return sl, risk, "fixed_atr", entry


def backtest_mixed_tier_signals(
    df: pd.DataFrame,
    *,
    rr: float,
    fixed_risk_atr: float,
    swing_lookback: int,
    atr_buffer: float,
    min_swing_risk_atr: float,
    max_swing_risk_atr: float,
    max_bars: int,
    cooldown_bars: int,
    start_bar: int,
    point_size: float,
    ...
    for signal_idx, side, tier, reason in signals:
        ...
        if tier in {"HIGH", "STANDARD"}:
            stop = swing_stop(
                ...
                min_risk_atr=min_swing_risk_atr,
                max_risk_atr=max_swing_risk_atr,
            ...
            stop = fixed_atr_stop(
                ...
                entry_idx=entry_idx,
                side=side,
                risk_atr=fixed_risk_atr,
                point_size=point_size,
                spread_multiplier=spread_multiplier,
            )
            sl_mode = "fixed_atr_sl"

        if stop is None:
            continue

        sl, risk, stop_anchor_type, entry = stop
        atr = float(df.at[signal_idx, "atr14"])
        ...
        for j in range(entry_idx, min(entry_idx + max_bars, len(df) - 1) + 1):
            ...
            spread = spread_price(df.iloc[j], point_size=point_size, spread_multiplier=spread_multiplier)
            bid_high = high_mid - spread / 2
            bid_low = low_mid - spread / 2
            ask_high = high_mid + spread / 2
            ask_low = low_mid + spread / 2

            if side == "BUY":
                # BUY enters at ask, exits at bid. SL/TP are evaluated on bid.
                if bid_low <= sl:
                    exit_idx = j
                    exit_price = sl
                    exit_reason = "sl"
                ...
                # SELL enters at bid, exits at ask. SL/TP are evaluated on ask.
                if ask_high >= sl:
                    exit_idx = j
                    exit_price = sl
                    exit_reason = "sl"
        ...
        entry_spread = spread_price(df.iloc[entry_idx], point_size=point_size, spread_multiplier=spread_multiplier)
        trades.append(
            {
                "notify_tier": tier,
                "sl_mode": sl_mode,
                ...
                "entry_mid_price": float(df.at[entry_idx, "open"]),
                "entry_spread_price": float(entry_spread),
                "sl": float(sl),
                "tp": float(tp),
                "risk": float(risk),
                "risk_atr_ratio": float(risk_atr_ratio),
...
def summarize_group(trades: pd.DataFrame, *, group_name: str, group_value: str) -> dict[str, object]:
    ...
    return {
        ...
        "avg_entry_spread_price": float(trades["entry_spread_price"].mean()),
...
def summarize_tiers(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for mode in ["swing_sl", "fixed_atr_sl"]:
        rows.append(summarize_group(trades[trades["sl_mode"] == mode].copy(), group_name="sl_mode", group_value=mode))
    return pd.DataFrame(rows)


def write_rules(path: Path, *, rr: float, fixed_risk_atr: float, swing_lookback: int, atr_buffer: float, min_swing_risk_atr: float, max_swing_risk_atr: float, point_size: float, spread_multiplier: float) -> None:
    content = f"""# BTCUSD# B/RCI Mixed SL Spread-Aware Backtest Rules

This backtest uses hierarchical BTC B/RCI tiers and applies different SL methods by tier.
...
HIGH / STANDARD:
  recent swing SL
  BUY  = recent {swing_lookback}-bar low - ATR14 * {atr_buffer}
  SELL = recent {swing_lookback}-bar high + ATR14 * {atr_buffer}
  min_swing_risk_atr = {min_swing_risk_atr}
  max_swing_risk_atr = {max_swing_risk_atr}

WIDE:
  fixed ATR SL
  risk = ATR14 * {fixed_risk_atr}
...
point_size = {point_size}
spread_multiplier = {spread_multiplier}
spread_price = spread_points * point_size * spread_multiplier

BUY:
  entry = open_mid + spread_price / 2
  SL/TP trigger uses bid high/low = mid high/low - spread_price / 2

SELL:
  entry = open_mid - spread_price / 2
  SL/TP trigger uses ask high/low = mid high/low + spread_price / 2
```
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest BTCUSD# B/RCI tiers with mixed SL and spread-aware execution.")
    ...
    parser.add_argument("--fixed-risk-atr", type=float, default=1.0)
    parser.add_argument("--swing-lookback", type=int, default=8)
    parser.add_argument("--atr-buffer", type=float, default=0.05)
    parser.add_argument("--min-swing-risk-atr", type=float, default=0.20)
    parser.add_argument("--max-swing-risk-atr", type=float, default=3.00)
    ...
    parser.add_argument("--point-size", type=float, default=0.01, help="Price units per spread point. Check broker symbol spec.")
    parser.add_argument("--spread-multiplier", type=float, default=1.0, help="Use >1 for conservative spread/slippage stress test.")
    ...
    trades = backtest_mixed_tier_signals(
        df,
        rr=args.rr,
        fixed_risk_atr=args.fixed_risk_atr,
        swing_lookback=args.swing_lookback,
        atr_buffer=args.atr_buffer,
        min_swing_risk_atr=args.min_swing_risk_atr,
        max_swing_risk_atr=args.max_swing_risk_atr,
    ...
    print("RR:", args.rr, "fixed_risk_atr:", args.fixed_risk_atr)
    print(
        "Swing SL:",
        "lookback",
        args.swing_lookback,
        ...
        args.atr_buffer,
        "risk_atr range",
        args.min_swing_risk_atr,
        "to",
        args.max_swing_risk_atr,
    )
    ...
    display_cols = [
        ...
        "avg_entry_spread_price",
        "avg_spread_to_risk",
        "trades_per_month",
    ]
    print_table("BTCUSD# B/RCI MIXED SL SPREAD-AWARE SUMMARY", summary[display_cols] if not summary.empty else summary)
