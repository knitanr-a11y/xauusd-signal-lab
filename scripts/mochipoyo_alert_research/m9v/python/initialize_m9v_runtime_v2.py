...
def main() -> int:
    ...
    try:
        ...
        start = latest["M1"]
        # M1 is the coverage anchor. Other closed-bar CSVs may trail it by less
        # than two native bars, but cannot be ahead of it.
        for timeframe, value in latest.items():
            lag = (start - value).total_seconds()
            if lag < 0:
                raise core.M9VContractError(
                    f"{timeframe} is ahead of M1 coverage during fresh-start freeze; rerun after M1 catches up"
                )
            if lag > MAX_LAG_SECONDS[timeframe]:
                raise core.M9VContractError(
                    f"live CSV coverage is not synchronized enough for fresh start: {timeframe} lag={lag}s max={MAX_LAG_SECONDS[timeframe]}s"
                )

        frozen_prefix: dict[str, Any] = {}
        state_at_start: dict[str, Any] = {}
        for timeframe, filename in contract["data"]["live_file_map"].items():
            path = args.data_root / str(filename)
            frozen_rows = int(after[timeframe]["row_count"])
            frozen_prefix[timeframe] = core.prefix_fingerprint_rows(path, frozen_rows)
            if timeframe in ("M5", "M15", "H1", "H4"):
                bars = core.m9p.load_bars(path)
                if len(bars) != frozen_rows:
                    raise core.M9VContractError(f"{timeframe} row count changed during state freeze")
        ...
        runtime = {
            ...
            "runtime_contract_version": core.RUNTIME_CONTRACT_VERSION,
            "created_at_utc": now,
            ...
            "frozen_row_prefixes": frozen_prefix,
            "prefix_semantics": "IMMUTABLE_ROWS_PRESENT_AT_INITIALIZATION; LATER STRICTLY-ASCENDING APPENDS ARE ALLOWED",
