...
STAGE = "M10W33_PREREGISTERED_LOW_ATR_SCALE_NORMALIZED_ENTRY_EVALUATION_AUDIT_ONLY"
CONTRACT = ROOT / "config" / "mochipoyo_alert_research" / "m10w32_scale_normalized_entry_preregistration_20260728.json"
TIME_FORMAT = c.TIME_FORMAT
HORIZON = timedelta(minutes=240)
FIXED_SPREAD_USD = 0.20
EXPECTED_FEATURE_SHA256 = "8ad229791f1fb1a14760e715c65d03b4431bcea5f261155cd49dfb028175cc52"
...
def load_feature_rows(path: Path) -> list[dict[str, Any]]:
    if not path.is_file():
        raise RuntimeError(f"M10W31 scale-normalized feature rows missing: {path}")
    actual_hash = sha256(path)
    if actual_hash != EXPECTED_FEATURE_SHA256:
        raise RuntimeError(f"M10W31 feature SHA mismatch: {actual_hash} expected={EXPECTED_FEATURE_SHA256}")
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != EXPECTED_FEATURE_ROWS:
        raise RuntimeError(f"M10W31 row count drift: {len(rows)} expected={EXPECTED_FEATURE_ROWS}")
    decisions = [str(row.get("decision_time", "")) for row in rows]
    if len(decisions) != len(set(decisions)):
        raise RuntimeError("duplicate decision_time in M10W31 feature rows")
    forbidden = {
        "actual_return_bps",
        "fixed0p20_return_bps",
        "trade_id",
        "status",
        "scheduled_exit_time",
        "win",
...
def matches(row: dict[str, Any], family: str) -> bool:
    if family == "SNRI1_LONG_M5_NORMALIZED_RANGE_IMPULSE":
        return (
            f(row, "m5_range3_over_h1_atr14") >= 0.40
            and f(row, "m5_ret3_over_h1_atr14") > 0.0
            and f(row, "m5_close_location") >= (2.0 / 3.0)
        )
    if family == "SNRC1_LONG_M15_NORMALIZED_RECLAIM_CONTINUATION":
        location = f(row, "m15_close_minus_ema20_over_h1_atr14")
        return (
            location >= 0.0
            and location <= 0.50
            and f(row, "m5_ret3_over_h1_atr14") > 0.0
            and f(row, "m5_close_location") >= 0.60
        )
    if family == "SNDX1_LONG_DUAL_SCALE_NORMALIZED_EXPANSION":
        return (
            f(row, "m5_range3_over_h1_atr14") >= 0.40
            and f(row, "m1_range5_over_h1_atr14") >= 0.20
            and f(row, "m1_ret5_over_h1_atr14") > 0.0
            and f(row, "m1_close_location") >= 0.60
        )
...
def metrics(values: list[float]) -> dict[str, Any]:
    ...
    return {
        ...
        "max_losing_streak": maximum_streak,
    }
...
def classify(blocks: dict[str, Any], gates: dict[str, Any]) -> str:
    ...
    if (
        ...
        and cost2_pf >= float(strong["minimum_extra2bps_all_pf"])
    ...
    if (
        ...
        and cost2_pf >= float(robust["minimum_extra2bps_all_pf"])
        and all(net > 0 for net in nets)
    ):
...
def main() -> int:
    ...
    output_root = local_root / "outputs" / "M10W33"
    try:
        contract = load_json(CONTRACT)
        if (
            contract.get("stage") != "M10W32_LOW_ATR_SCALE_NORMALIZED_ENTRY_PREREGISTRATION_AUDIT_ONLY"
            or contract.get("status") != "HYPOTHESES_FROZEN_BEFORE_OUTCOME_EVALUATION"
        ):
            raise RuntimeError("unexpected M10W32 contract")
        feature_path = local_root / "outputs" / "M10W31" / "LATEST" / "02_scale_normalized_feature_rows.csv"
        ...
        summary = {
            ...
            "stage": STAGE,
            "status": "PASS_PREREGISTERED_SCALE_NORMALIZED_EVALUATION_AUDIT_ONLY",
            ...
            "target_regime": contract["exact_parent_cohort"]["target"],
            "families": family_results,
            "historical_interpretation": "EXPLORATORY_NOT_FRESH_SUPPORT",
        ...
        (archive / "00_READ_ME_FIRST.txt").write_text(
            "M10W33 evaluates the three M10W32 preregistered scale-normalized low-ATR causal-NEITHER families without tuning. Historical results are exploratory and never fresh support. M10W29 families remain closed.\n",
            encoding="utf-8",
        )
        ...
        print("[M10W33 PASS] preregistered scale-normalized evaluation complete")
        ...
        (output_root / "LATEST_ERROR.txt").write_text(
            f"{type(exc).__name__}: {exc}\n",
            encoding="utf-8",
        )
        print(f"[M10W33 BLOCKED] {type(exc).__name__}: {exc}")
        return 2
