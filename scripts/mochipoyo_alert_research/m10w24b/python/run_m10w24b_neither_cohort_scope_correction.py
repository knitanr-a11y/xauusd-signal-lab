...
def load_coverage_neither_times(path: Path) -> set[str]:
    if not path.is_file():
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            seen.add(decision)
            if str(row["coverage_class"]) == "NEITHER":
                neither.add(decision)
    return neither
...
def main() -> int:
    ...
    try:
        ...
        neither_times = load_coverage_neither_times(coverage_path)
        corrected_rows = [row for row in feature_rows if str(row["decision_time"]) in neither_times]
        if not corrected_rows:
            raise RuntimeError("corrected NEITHER target cohort is empty")

        broad_times = {str(row["decision_time"]) for row in feature_rows}
        ...
        for family in contract["families"]:
            ...
            classification = base.classify(blocks, gates)
        ...
        (archive / "00_READ_ME_FIRST.txt").write_text(
            "M10W24B restores the original M10W17 NEITHER cohort by exact decision_time join. M10W23 formulas, thresholds, horizon, execution and decision tiers are unchanged.\n",
            encoding="utf-8",
        )
        ...
        print(f"[M10W24B PASS] broader={len(feature_rows)} neither={len(corrected_rows)} advancing={advancing if advancing else 'NONE'}")
