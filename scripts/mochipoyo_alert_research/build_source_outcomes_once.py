...
from source_outcome_builder import rebuild_source_outcomes

SCRIPT_DIR = Path(__file__).resolve().parent
SCHEMA_PATH = SCRIPT_DIR / "schema.sql"
REPORT_NAME = "latest_source_outcome_build_result.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build audit-only source-alert MFE/MAE and source EXIT outcomes."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        payload = {
            "status": "PASS",
            "stage": "M6A_SOURCE_ALERT_PATH_OUTCOMES",
            "audit_only": True,
            "dry_run": True,
            "database_write_performed": True,
            "derived_virtual_entry_rows_rebuilt": True,
            "derived_outcome_rows_rebuilt": True,
            "derived_path_metric_rows_rebuilt": True,
            ...
            **result,
            "path_contract": {
                "entry_price_basis": "TradingView source close",
                "exit_price_basis": "TradingView source EXIT close",
                "path_price_basis": "MT5 M1 OHLC with audited per-entry offset",
                "entry_minute": "excluded in full",
                "exit_minute": "excluded in full",
                "source_exit_price": "included as terminal point",
                "reason": (
                    "M1 OHLC cannot identify whether an intraminute extreme "
                    "occurred before or after the source event."
                ),
                "dst_rule": (
                    "entry and source EXIT offsets must match; otherwise fail closed"
                ),
            },
            "sample_size_warning": (
                "These are descriptive audit metrics. High-expectancy rules are not "
                "approved from the current small completed-episode sample."
            ),
        ...
        print(
            f"[ERROR] Source outcome build failed: "
            f"{type(exc).__name__}: {exc}"
        )
        print(
            "[SAFE] Raw alerts, episodes, alignment, features, and the previous "
            "successful M6A rows were preserved."
        )
        return 1
    finally:
        connection.close()
