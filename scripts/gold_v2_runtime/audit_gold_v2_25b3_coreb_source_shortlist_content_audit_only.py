#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""GOLD V2 25B3 CoreB source shortlist content audit-only.

Reads the 25B2 priority shortlist and inspects only the shortlisted files.
This is content/schema/linkage audit only. It does not replay CoreB, reconstruct
same_count, infer membership, fit cluster_id, mutate artifacts, call AI, send
Discord, place MT5 orders, connect live hooks, or enable final signals.
"""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="25B3 CoreB source shortlist content audit-only")
...
def read_json_any(path: Path) -> Any:
    txt = lp(path).read_text(encoding="utf-8-sig")
...
def csv_profile(path: Path, normalized_path: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    row: dict[str, Any] = {
        ...
        "same_count_min": float(pd.to_numeric(df["same_count"], errors="coerce").min()) if "same_count" in cols and len(df) else "",
        "same_count_max": float(pd.to_numeric(df["same_count"], errors="coerce").max()) if "same_count" in cols and len(df) else "",
        "same_count_ge15_rows": int((pd.to_numeric(df["same_count"], errors="coerce") >= 15).sum()) if "same_count" in cols else "",
    }
    samples = []
...
def classify_evidence(normalized_path: str, triage_class: str, csv_row: dict[str, Any] | None, json_row: dict[str, Any] | None) -> dict[str, Any]:
    ...
    proves_membership = False
    if csv_row:
        proves_membership = bool(csv_row.get("has_member_rows"))
    if json_row:
        proves_membership = proves_membership or False
    ...
    return {
        ...
        "proves_row_level_membership_semantics_now": bool(proves_membership),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    s25b2 = json.loads(lp(summary_path).read_text(encoding="utf-8"))
    ...
    for _, sr in shortlist.iterrows():
        ...
        linkage_rows.append(classify_evidence(npath, str(sr.get("triage_class", "")), csv_row, json_row))
    ...
    next_plan = pd.DataFrame([
        ...
        {"rank": 3, "next_step": "CoreB live evaluator", "purpose": "Still blocked until replay/parity proves 125 exact rows and same_count/membership parity.", "allowed_now": False, "blocked_actions": "final signal / Discord / MT5 / AI / live hook"},
    ])
    ...
    source_roles = {str(r["evidence_role"]): int(v) for r, v in linkage.groupby("evidence_role").size().reset_index(name="rows").set_index("evidence_role").iterrows()}
    summary = {
    ...
    report = "\n".join([
        ...
        "## Boundary",
        "",
        "25B3 inspects only the 25B2 priority shortlist. It does not replay CoreB, reconstruct same_count, infer membership, fit cluster_id, mutate source artifacts, call AI APIs, send Discord, place MT5 orders, connect live hooks, or enable final signals.",
        "",
        "## Input audit",
        "",
        md_table(inputs),
    ...
    print(json.dumps({"status": status, "output_dir": str(out_dir), "shortlist_rows": int(len(shortlist)), "profiled_csv_files": int(len(csv_profile_df)), "profiled_json_files": int(len(json_profile_df)), "next_recommended_step": summary["next_recommended_step"]}, ensure_ascii=False, indent=2, allow_nan=False))
