...
STEP = "24E_SOURCE_RECOVERY_ARTIFACT_LIST_INTAKE_AUDIT_ONLY_HARDENED"
...
def normalize_hash(value: Any) -> str:
    return str(value).strip().lower().replace("sha256:", "").replace("blob:", "")
...
def resolve_artifact_path(raw_path: Any) -> tuple[Path | None, str, bool]:
    text = str(raw_path).strip()
    if not text:
        return None, "EMPTY_ARTIFACT_PATH", False
    ...
    for c in unique_candidates:
        try:
            if long_path(c).exists() and long_path(c).is_file():
                return c.resolve(), "FOUND", True
        except Exception:
            continue
    return unique_candidates[0], "NOT_FOUND", False
...
def validate_artifact_input(template: pd.DataFrame, user_input: pd.DataFrame | None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    rows = []
    stats = {
        "artifact_list_supplied": user_input is not None,
        "unknown_ids": [],
        "duplicate_ids": [],
        "missing_ids": [],
        "valid_rows": 0,
        "invalid_rows": 0,
        "forbidden_true_rows": 0,
        "artifact_exists_rows": 0,
        "artifact_hash_match_rows": 0,
    }
    if user_input is None:
        for _, t in template.iterrows():
            rows.append({
                ...
                "resolved_artifact_path": "", "artifact_exists": False, "declared_artifact_hash": "", "computed_sha256": "", "computed_git_blob_sha1": "", "artifact_hash_matches": False,
    ...
    if "intake_id" not in user_input.columns:
        ...
        for _, t in template.iterrows():
            rows.append({
                ...
                "artifact_category": t.get("artifact_category", ""), "artifact_path_present": False, "artifact_hash_present": False,
                "artifact_role_present": False, "source_identity_scope_present": False, "upstream_sot_reference_present": False,
                "quarantine_note_present_or_not_required": False, "resolved_artifact_path": "", "artifact_exists": False,
                "declared_artifact_hash": "", "computed_sha256": "", "computed_git_blob_sha1": "", "artifact_hash_matches": False,
                "execution_approved": False, "source_recovery_approved": False, "valid_for_24f_review": False,
                "status": "INVALID_INPUT_MISSING_INTAKE_ID_COLUMN",
            })
        stats["invalid_rows"] = len(rows)
        return pd.DataFrame(rows), stats
    ...
    for _, t in template.iterrows():
        ...
        if r is None:
            ...
            rec = False
            resolved = None
            resolve_status = "MISSING_REQUIRED_INTAKE_ROW"
            artifact_exists = False
            declared_hash = ""
            computed_sha256 = ""
            computed_git_blob_sha1 = ""
            hash_match = False
            valid = False
            ...
            resolved, resolve_status, artifact_exists = resolve_artifact_path(r.get("artifact_path", ""))
            declared_hash = normalize_hash(r.get("artifact_hash", ""))
            computed_sha256 = ""
            computed_git_blob_sha1 = ""
            hash_match = False
            if artifact_exists and resolved is not None:
                try:
                    computed_sha256 = sha256_file(resolved)
                    computed_git_blob_sha1 = git_blob_sha1_file(resolved)
                    hash_match = declared_hash in {computed_sha256.lower(), computed_git_blob_sha1.lower()}
                except Exception as exc:
                    resolve_status = f"HASH_COMPUTE_ERROR:{exc}"
            valid = all(present.values()) and q_ok and artifact_exists and hash_match and not ex and not rec
            if valid:
                status = "VALID_FOR_24F_AUDIT_ONLY_REVIEW_HASH_VERIFIED"
            elif not all(present.values()) or not q_ok:
                status = "INVALID_OR_INCOMPLETE_ARTIFACT_ROW"
            elif not artifact_exists:
                status = "INVALID_ARTIFACT_PATH_NOT_FOUND"
            elif not hash_match:
                status = "INVALID_ARTIFACT_HASH_MISMATCH"
            else:
                status = "INVALID_ARTIFACT_ROW"
        if ex or rec:
            stats["forbidden_true_rows"] += 1
        stats["artifact_exists_rows"] += int(artifact_exists)
        stats["artifact_hash_match_rows"] += int(hash_match)
        ...
        rows.append({
            ...
            "resolved_artifact_path": str(resolved) if resolved is not None else "", "artifact_resolve_status": resolve_status,
            "artifact_exists": artifact_exists, "declared_artifact_hash": declared_hash, "computed_sha256": computed_sha256,
            "computed_git_blob_sha1": computed_git_blob_sha1, "artifact_hash_matches": hash_match,
...
def build_safety_matrix(summary24d: dict[str, Any], ok: bool, inputs_ok: bool, validated: bool) -> pd.DataFrame:
    rows = []
    ...
    add("artifact_existence_and_hash_required_for_24f", validated, True, bool(validated) or True, "If false, 24F remains blocked rather than STOP.")
...
def build_report(now: str, status: str, input_audit: pd.DataFrame, checks: pd.DataFrame, template: pd.DataFrame, intake: pd.DataFrame, gates: pd.DataFrame, safety: pd.DataFrame, summary: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "## Boundary", "", "- 24E is audit-only.", "- 24E reads 24D audited artifacts as the source of truth.", "- 24E writes/fills/validates artifact list intake only.", "- 24E verifies referenced artifact existence and hash before allowing 24F.", "- 24E does not approve or execute source recovery.", "- Source identity finalization/recovery, live evaluator, final signal, Discord, MT5, AI API, and live hook remain blocked.", "- Old GOLD/DISC8 remain quarantined.", "",
        "## Outcome", "", f"- Total STOP rows: `{summary.get('total_stop_rows')}`", f"- Artifact list supplied: `{summary.get('artifact_list_supplied')}`", f"- Artifact list validated: `{summary.get('artifact_list_validated')}`", f"- Artifact exists rows: `{summary.get('artifact_exists_rows')}`", f"- Artifact hash match rows: `{summary.get('artifact_hash_match_rows')}`", f"- Next recommended step: `{summary.get('next_recommended_step')}`", "",
...
def main() -> int:
    ...
    if inputs_ok:
        ...
        checks.extend([
            ...
            check_row("24E-C006", "24D required next allowed only 24E", allowed_after_24d, ["24E_SOURCE_RECOVERY_ARTIFACT_LIST_INTAKE_AUDIT_ONLY"], allowed_after_24d == ["24E_SOURCE_RECOVERY_ARTIFACT_LIST_INTAKE_AUDIT_ONLY"]),
    ...
    intake_df, stats = validate_artifact_input(template, user_df) if inputs_ok else (pd.DataFrame(), {"artifact_list_supplied": False, "valid_rows": 0, "invalid_rows": 0, "forbidden_true_rows": 0, "unknown_ids": [], "duplicate_ids": [], "missing_ids": [], "artifact_exists_rows": 0, "artifact_hash_match_rows": 0})
    if stats.get("artifact_list_supplied"):
        checks.extend([
            ...
            check_row("24E-C016", "Supplied artifacts exist for all 3 rows", stats["artifact_exists_rows"], 3, stats["artifact_exists_rows"] == 3),
            check_row("24E-C017", "Supplied artifact hashes match all 3 rows", stats["artifact_hash_match_rows"], 3, stats["artifact_hash_match_rows"] == 3),
        ])
    checks_df = pd.DataFrame(checks)
    validated = bool(
        inputs_ok and stats.get("artifact_list_supplied") and stats.get("valid_rows") == 3 and stats.get("invalid_rows") == 0
        and stats.get("forbidden_true_rows") == 0 and stats.get("artifact_exists_rows") == 3 and stats.get("artifact_hash_match_rows") == 3
        and len(stats.get("unknown_ids", [])) == 0 and len(stats.get("duplicate_ids", [])) == 0 and len(stats.get("missing_ids", [])) == 0
    )
    preliminary_ok = inputs_ok and stop_rows(checks_df) == 0
    safety_df = build_safety_matrix(summary24d, preliminary_ok, inputs_ok, validated)
    ...
    summary = {
        ...
        "artifact_existence_and_hash_verification_enforced": True,
        ...
        "artifact_exists_rows": int(stats.get("artifact_exists_rows", 0)), "artifact_hash_match_rows": int(stats.get("artifact_hash_match_rows", 0)),
