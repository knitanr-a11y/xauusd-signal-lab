...
COMPONENT = "RANGE96_REFINED"
OUT_DIR_NAME = "gold_v2_17c_range96_refined_reconciliation_audit_only"
REPORT_NAME = "GOLD_V2_17C_RANGE96_REFINED_RECONCILIATION_AUDIT_ONLY_REPORT.md"
EXPECTED_RULE_ROWS = 51
EXPECTED_COMBINED_ROWS = 117
EXTERNAL_ACTIONS = {
...
def matrix_checks(matrix_17a: pd.DataFrame, matrix_17b: pd.DataFrame) -> pd.DataFrame:
    ...
    if a.empty:
        rows.append(["17C-17A-000", "17A component row exists", "missing", COMPONENT, "STOP"])
    else:
        row = a.iloc[0]
        ...
        for key, exp in expected:
            ...
            if isinstance(exp, int):
                obs = int(obs)
            rows.append([f"17C-17A-{key}", f"17A {key}", obs, exp, "PASS" if obs == exp else "STOP"])
    if b.empty:
        rows.append(["17C-17B-000", "17B component row exists", "missing", COMPONENT, "STOP"])
    else:
        row = b.iloc[0]
        expected = [("planned_step", "17C"), ("planning_status", "PLAN_READY"), ("rule_ledger_rows", EXPECTED_RULE_ROWS), ("combined_ledger_rows", EXPECTED_COMBINED_ROWS), ("live_evaluator_allowed", False), ("final_signal_allowed", False)]
        for key, exp in expected:
            obs = bool_value(row.get(key, False)) if isinstance(exp, bool) else row.get(key, "")
            if isinstance(exp, int):
                obs = int(obs)
            rows.append([f"17C-17B-{key}", f"17B {key}", obs, exp, "PASS" if obs == exp else "STOP"])
...
def key_reconciliation(rule_rows: pd.DataFrame, combined_rows: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    summary = pd.DataFrame([
        ["rule_unique_keys", len(rule_keys), "<=51", "INFO", "duplicates audited"],
        ["combined_unique_keys", len(combined_keys), "<=117", "INFO", "combined extras allowed"],
        ["rule_keys_missing_in_combined", len(missing), 0, "PASS" if not missing else "STOP", "all rule keys must exist in combined"],
        ["combined_extra_keys_not_in_rule", len(extra), "allowed/audited", "INFO", "117-row source extras"],
...
def main() -> int:
    ...
    audit = input_audit()
    write_csv(audit, out / "gold_v2_17c_input_audit.csv")
    missing = audit[audit["required"].eq(True) & ~audit["exists"].eq(True)]
    if len(missing):
        status = "RANGE96_REFINED_RECONCILIATION_MISSING_INPUTS_AUDIT_ONLY"
        blockers = pd.DataFrame([["17C-BINPUT", COMPONENT, "HARD", "OPEN", "required inputs", "Missing: " + "|".join(missing["role"].astype(str))], ["17C-B099", "SAFETY", "SAFETY", "OPEN", "external actions", "All external actions remain false."]], columns=["blocker_id", "component", "severity", "status", "blocked_item", "required_resolution"])
        write_csv(blockers, out / "gold_v2_17c_range96_blockers.csv")
        write_json(out / "gold_v2_17c_range96_reconciliation_summary.json", {"created_utc": now, "step": STEP, "component": COMPONENT, "status": status, "audit_only": True, "missing_required_inputs": missing[["role", "path"]].to_dict("records"), "medium_live_evaluator_allowed": False, "final_signal_allowed": False, "external_actions": EXTERNAL_ACTIONS})
        (out / REPORT_NAME).write_text("\n".join(["# GOLD V2 17C RANGE96_REFINED reconciliation audit-only report", "", f"Created UTC: {now}", f"Status: `{status}`", "", markdown_table(audit), "", markdown_table(blockers)]), encoding="utf-8")
    ...
    write_csv(rule_rows, out / "gold_v2_17c_range96_rule_ledger_source_rows.csv")
    write_csv(combined_rows, out / "gold_v2_17c_range96_combined_ledger_source_rows.csv")
    write_csv(source_checks, out / "gold_v2_17c_range96_source_extraction_checks.csv")
    write_csv(matrix, out / "gold_v2_17c_17a_17b_consistency_checks.csv")
    write_csv(key_summary, out / "gold_v2_17c_range96_key_reconciliation_summary.csv")
    write_csv(missing_keys, out / "gold_v2_17c_range96_rule_keys_missing_in_combined.csv")
    write_csv(extra_keys, out / "gold_v2_17c_range96_combined_extra_keys_not_in_rule.csv")
    ...
    write_csv(safety, out / "gold_v2_17c_safety_matrix.csv")
    ...
    status = "RANGE96_REFINED_SOURCE_RECONCILIATION_READY_FOR_CANDIDATE_SOURCE_FREEZE_AUDIT_ONLY" if ok else "RANGE96_REFINED_SOURCE_RECONCILIATION_STOPPED_AUDIT_ONLY"
    ...
    next_step = "17C2_RANGE96_REFINED_CANDIDATE_RULE_FREEZE_AUDIT_ONLY_OR_17D_VOL_TRMEAN32_REFINED_RECONCILIATION_AUDIT_ONLY" if ok else "STOP_REVIEW_17C_SOURCE_RECONCILIATION_OUTPUTS"
    decision = pd.DataFrame([["17C-C001", "rule ledger rows", len(rule_rows), EXPECTED_RULE_ROWS, "PASS" if len(rule_rows) == EXPECTED_RULE_ROWS else "STOP"], ["17C-C002", "combined ledger rows", len(combined_rows), EXPECTED_COMBINED_ROWS, "PASS" if len(combined_rows) == EXPECTED_COMBINED_ROWS else "STOP"], ["17C-C003", "17A/17B consistency", matrix[matrix["status"].eq("STOP")].empty, True, "PASS" if matrix[matrix["status"].eq("STOP")].empty else "STOP"], ["17C-C004", "rule keys present in combined", key_summary[key_summary["status"].eq("STOP")].empty, True, "PASS" if key_summary[key_summary["status"].eq("STOP")].empty else "STOP"], ["17C-C005", "candidate source freeze preview", candidate_status, "audit-only preview, not live", "PASS" if ok else "STOP"]], columns=["check_id", "check", "observed", "expected", "status"])
    write_csv(decision, out / "gold_v2_17c_range96_decision_matrix.csv")
    freeze = {"created_utc": now, "step": STEP, "component": COMPONENT, "audit_only": True, "candidate_freeze_type": "SOURCE_ROW_IDENTITY_FREEZE_PREVIEW_ONLY", "candidate_status": candidate_status, "expected_counts": {"rule_ledger_rows": EXPECTED_RULE_ROWS, "combined_ledger_rows": EXPECTED_COMBINED_ROWS}, "observed_counts": {"rule_ledger_rows": len(rule_rows), "combined_ledger_rows": len(combined_rows)}, "component_filter": {"rule_component_col": rule_component_col, "combined_component_col": combined_component_col, "equals": COMPONENT}, "rule_source_row_hashes": rule_rows["source_row_hash"].astype(str).tolist() if "source_row_hash" in rule_rows.columns else [], "combined_source_row_hashes": combined_rows["source_row_hash"].astype(str).tolist() if "source_row_hash" in combined_rows.columns else [], "hard_warnings": ["Not executable live rule", "No OHLC rediscovery", "No approximate RANGE96_REFINED condition", "Feature/asof parity not proven", "External actions disabled"], "medium_live_evaluator_allowed": False, "final_signal_allowed": False, "external_actions": EXTERNAL_ACTIONS}
    write_json(out / "gold_v2_17c_range96_candidate_source_freeze_preview.json", freeze)
    index_cols = ["source_role", "source_row_number_1based", "source_key", "strategy_id", "source_row_hash"]
    write_csv(pd.concat([rule_rows[[c for c in index_cols if c in rule_rows.columns]], combined_rows[[c for c in index_cols if c in combined_rows.columns]]], ignore_index=True), out / "gold_v2_17c_range96_candidate_source_freeze_index.csv")
    blocks = []
    for _, row in source_checks[source_checks["status"].eq("STOP")].iterrows():
        blocks.append(["17C-B001", COMPONENT, "HARD", "OPEN", "source row extraction", f"{row['source_role']} observed={row['observed_rows']} expected={row['expected_rows']}"])
    for _, row in matrix[matrix["status"].eq("STOP")].iterrows():
        blocks.append(["17C-B17AB", COMPONENT, "HARD", "OPEN", "17A/17B consistency", f"{row['check']}: observed={row['observed']} expected={row['expected']}"])
    for _, row in key_summary[key_summary["status"].eq("STOP")].iterrows():
        blocks.append(["17C-BKEY", COMPONENT, "HARD", "OPEN", "key reconciliation", f"{row['metric']}: observed={row['observed']} expected={row['expected']}"])
    blocks += [["17C-B010", COMPONENT, "HARD", "OPEN", "executable rule freeze", "17C writes source-row freeze preview only; executable rule/load-smoke requires later audit."], ["17C-B020", "MEDIUM_FULL_SET", "HARD", "OPEN", "full MEDIUM set", "17D VOL_TRMEAN32_REFINED reconciliation is still required."], ["17C-B099", "SAFETY", "SAFETY", "OPEN", "external actions", "Keep final_signal_allowed=false, Discord=false, MT5=false, AI=false, live_hook=false. NO_SIGNAL must not notify Discord."]]
