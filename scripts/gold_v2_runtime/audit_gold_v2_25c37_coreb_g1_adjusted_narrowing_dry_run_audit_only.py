...
STATUS_EXACT="COREB_G1_ADJUSTED_NARROWING_DRY_RUN_COMPLETED_AUDIT_ONLY_EXACT_MATCH_REVIEW_REQUIRED"
STOP="25C37_STOP_MISSING_INPUT_AUDIT_ONLY"
OUT_DIR="gold_v2_25c37_coreb_g1_adjusted_narrowing_dry_run_audit_only"
...
def main(argv:Optional[Sequence[str]]=None)->int:
    ...
    req={
        "s36":fx_outputs()/IN36/"02_25c36_coreb_g1_over_narrowing_adjustment_plan_summary.json",
        "bundles":fx_outputs()/IN36/"04_25c36_adjusted_bundle_candidate_matrix.csv",
        "members":fx_outputs()/IN36/"05_25c36_adjusted_bundle_membership.csv",
    ...
    write_csv(out/"03_25c37_input_audit.csv", ia)
    if not bool(ia["exists"].all()):
        write_json(out/"02_25c37_coreb_g1_adjusted_narrowing_dry_run_summary.json", {"created_utc":datetime.now(timezone.utc).isoformat(),"step":STEP,"status":STOP,"total_stop_rows":int((ia["status"]=="STOP").sum())}); return 2
    s36=read_json(req["s36"])
    if not s36.get("requires_human_acceptance_before_25c37", False):
        write_json(out/"02_25c37_coreb_g1_adjusted_narrowing_dry_run_summary.json", {"created_utc":datetime.now(timezone.utc).isoformat(),"step":STEP,"status":"25C37_STOP_25C36_ACCEPTANCE_CONTRACT_MISSING_AUDIT_ONLY","total_stop_rows":1}); return 2
    ...
    target_key=target[KEY].drop_duplicates(); signals=signals[signals["policy"].isin(selected)].copy()
    ...
    for _,b in bundles.iterrows():
        bid=str(b[id_col]); bname=str(b["bundle_name"])
        filters=set(members[members[id_col].astype(str).eq(bid)]["filter"].astype(str).tolist())
        for f in sorted(filters): contract_rows.append({"variant":bid+"_"+bname,"adjusted_bundle_id":bid,"bundle_name":bname,"filter":f,"simulated_exclusion":True,"conditions_changed":False})
    ...
    write_csv(out/"04_25c37_variant_filter_contract.csv", contract)
    write_csv(out/"05_25c37_variant_compare_matrix.csv", matrix)
    ...
    delta=pd.DataFrame(deltas); write_csv(out/"06_25c37_variant_delta_matrix.csv", delta)
    allcmp=pd.concat(compare_frames, ignore_index=True)
    write_csv(out/"07_25c37_variant_by_dataset_policy.csv", allcmp.groupby(["variant","dataset","policy","_merge"],dropna=False).size().reset_index(name="entry_rows"))
    mat=matrix[~matrix["variant"].eq("BASELINE_CURRENT")].copy()
    mat["left_only_reduction"]=int(base["left_only"])-mat["left_only"]
    mat["over_narrowing_score"]=(mat["right_only"]-int(base["right_only"]))+(int(base["both"])-mat["both"])
    mat["net_tradeoff_score"]=mat["left_only_reduction"]-mat["over_narrowing_score"]
    best=mat.sort_values(["net_tradeoff_score","left_only_reduction"], ascending=[False,False]).iloc[0]["variant"]
    write_csv(out/"08_25c37_best_variant_left_only_samples.csv", allcmp[allcmp["variant"].eq(best)&allcmp["_merge"].eq("left_only")].sort_values(KEY).head(200))
    exact=bool(mat["exact_match"].any())
    gates=pd.DataFrame([
        {"gate_id":"G001","gate":"25C36 accepted","observed":True,"status":"PASS"},
        {"gate_id":"G002","gate":"adjusted narrowing dry-run executed audit-only","observed":True,"status":"PASS"},
    ...
    write_csv(out/"09_25c37_acceptance_gate_matrix.csv", gates)
    nxt=pd.DataFrame([
        {"rank":1,"next_step":"25C38_COREB_G1_ADJUSTED_NARROWING_RESULT_REVIEW_AUDIT_ONLY","allowed_now":True,"purpose":"review adjusted bundle result"},
        {"rank":2,"next_step":"CoreB live evaluator","allowed_now":False,"purpose":"blocked"},
    ])
    write_csv(out/"10_25c37_next_step_plan.csv", nxt)
    unnecessary=["25C36 older reports if summary is available","full replay rows","full target rows"]
    necessary=["01_25c37_GOLD_V2_COREB_G1_ADJUSTED_NARROWING_DRY_RUN_AUDIT_ONLY_REPORT.md","02_25c37_coreb_g1_adjusted_narrowing_dry_run_summary.json","04_25c37_variant_filter_contract.csv","05_25c37_variant_compare_matrix.csv","06_25c37_variant_delta_matrix.csv","07_25c37_variant_by_dataset_policy.csv","08_25c37_best_variant_left_only_samples.csv","09_25c37_acceptance_gate_matrix.csv","10_25c37_next_step_plan.csv"]
    write_csv(out/"00_不要_25c37_file_request_list.csv", pd.DataFrame([{"section":"00_不要_貼らなくてOK","rank":i+1,"item":x} for i,x in enumerate(unnecessary)] + [{"section":"必要・貼ってほしい","rank":i+1,"item":x} for i,x in enumerate(necessary)]))
    best_row=matrix[matrix["variant"].eq(best)].iloc[0]; status=STATUS_EXACT if exact else STATUS
    summary={"created_utc":datetime.now(timezone.utc).isoformat(),"step":STEP,"status":status,"audit_only":True,"dry_run_executed":True,"condition_changed":False,"full_coreb_parity":False,"variant_count":int(len(mat)),"best_variant":str(best),"best_left_only":int(best_row["left_only"]),"best_right_only":int(best_row["right_only"]),"best_both":int(best_row["both"]),"any_exact_match":exact,"source_recovery_executed":False,"source_mutation_executed":False,"coreb_live_evaluator_unblocked":False,"next_recommended_step":"25C38_COREB_G1_ADJUSTED_NARROWING_RESULT_REVIEW_AUDIT_ONLY","total_stop_rows":0}
    write_json(out/"02_25c37_coreb_g1_adjusted_narrowing_dry_run_summary.json", summary)
    report="\n".join(["# GOLD V2 25C37 CoreB G1 adjusted narrowing dry-run audit-only report","",f"Created UTC: {summary['created_utc']}",f"Step: `{STEP}`",f"Status: `{status}`","","## Variant filter contract","",md_table(contract),"","## Variant compare matrix","",md_table(matrix),"","## Variant delta matrix","",md_table(delta),"","## Acceptance gates","",md_table(gates),"","## File request list","","```text","00_不要_貼らなくてOK",*[f"00-{i+1}. {x}" for i,x in enumerate(unnecessary)],"","必要・貼ってほしい",*[f"{i+1:02d}. {x}" for i,x in enumerate(necessary)],"```","","## Next step plan","",md_table(nxt),"","## Safety","","CoreB remains blocked. External/live actions remain off."])
    lp(out/"01_25c37_GOLD_V2_COREB_G1_ADJUSTED_NARROWING_DRY_RUN_AUDIT_ONLY_REPORT.md").write_text(report, encoding="utf-8")
