#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""GOLD V2 25C89 source-universe rule tuple membership audit-only.

Filters raw RR125 rows by full source-universe rule tuples rather than independent
sets of values, then rebuilds interval components and compares with CoreB top 125.
...
STEP = "25C89_SOURCE_UNIVERSE_RULE_TUPLE_MEMBERSHIP_AUDIT_ONLY"
OUT_DIR_NAME = "gold_v2_25c89_source_universe_rule_tuple_membership_audit_only"
...
def collect_rule_rows(obj: Any) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []

    def maybe_add(d: dict[str, Any]) -> None:
        keys = {"candidate_id", "origin_id", "variant", "added_filter_text", "rule_id"}
        if any(k in d for k in keys):
            rows.append({k: norm(d.get(k)) for k in keys})
    ...
    walk(obj)
    df = pd.DataFrame(rows).drop_duplicates() if rows else pd.DataFrame(columns=["candidate_id", "origin_id", "variant", "added_filter_text", "rule_id"])
    for c in ["candidate_id", "origin_id", "variant", "added_filter_text", "rule_id"]:
        if c not in df.columns:
            df[c] = ""
    return df[["rule_id", "candidate_id", "origin_id", "variant", "added_filter_text"]].drop_duplicates().reset_index(drop=True)
...
def tuple_filter_raw(raw: pd.DataFrame, rules: pd.DataFrame, mode_cols: list[str]) -> pd.DataFrame:
    r = raw[raw["policy"].astype(str).eq("RR125_from_RR1_rules")].copy()
    if rules.empty:
        return r.iloc[0:0].copy()
    valid_cols = [c for c in mode_cols if c in r.columns and c in rules.columns]
    if valid_cols != mode_cols:
        return r.iloc[0:0].copy()
    rule_tuples = rules[valid_cols].drop_duplicates()
    # Drop unusable tuples with all empty values, but allow partial empty in columns because original may omit one side.
    rule_tuples = rule_tuples[~rule_tuples.apply(lambda row: all(norm(v) == "" for v in row), axis=1)].copy()
    return r.merge(rule_tuples, on=valid_cols, how="inner")
...
def profit_val(s: pd.Series, rule: str) -> float | None:
...
def evaluate_mode(raw: pd.DataFrame, top: pd.DataFrame, rules: pd.DataFrame, mode_cols: list[str]) -> tuple[dict[str, Any], pd.DataFrame]:
    mode = "+".join(mode_cols)
    filt = tuple_filter_raw(raw, rules, mode_cols)
    ...
    for _, tr in top.iterrows():
        ...
        rec: dict[str, Any] = {"mode": mode, "dataset": tr.get("dataset"), "entry_time": tr.get("entry_time"), "cluster_id": tr.get("cluster_id"), "source_same_count": tr.get("same_count_num"), "source_rule_count": tr.get("source_rule_count_num"), "source_profit": tr.get("profit_num")}
        if cover.empty:
            rec.update({"recon_count": None, "recon_unique_origins": None, "same_count_match": False, "source_rule_count_match": False, "unique_origins_match": False})
        else:
            ...
            rec.update({"recon_component_id": component_id, "recon_count": len(members), "recon_unique_origins": members["origin_id"].nunique(dropna=True) if "origin_id" in members.columns else None})
            ...
            for rule in PROFIT_RULES:
                val = profit_val(members["profit_num"], rule)
                rec[f"profit_{rule}_match"] = abs(float(tr["profit_num"]) - val) <= 1e-6 if val is not None and pd.notna(tr["profit_num"]) else False
        rows.append(rec)
    df = pd.DataFrame(rows)
    sm = {"mode": mode, "tuple_cols": ";".join(mode_cols), "rule_tuple_count": int(rules[mode_cols].drop_duplicates().shape[0]) if all(c in rules.columns for c in mode_cols) else 0, "filtered_raw_rows": int(len(filt)), "component_rows": int(len(comp)), "top_rows": int(len(top)), "covered_rows": int(df["recon_count"].notna().sum()) if "recon_count" in df.columns else 0, "same_count_exact": int(df["same_count_match"].sum()) if "same_count_match" in df.columns else 0, "source_rule_count_exact": int(df["source_rule_count_match"].sum()) if "source_rule_count_match" in df.columns else 0, "unique_origins_exact": int(df["unique_origins_match"].sum()) if "unique_origins_match" in df.columns else 0}
    for rule in PROFIT_RULES:
        col = f"profit_{rule}_match"
        sm[f"profit_{rule}_exact"] = int(df[col].sum()) if col in df.columns else 0
...
def main() -> int:
    ...
    inv = inventory(paths)
    s88 = read_json(paths["25c88_summary.json"])
    rules = collect_rule_rows(read_json(paths["frozen_coreB_same_count_source_universe_20260604.json"]))
    ...
    summaries = []
    detail_frames = []
    for mode_cols in TUPLE_MODES:
        sm, df = evaluate_mode(raw, top, rules, mode_cols)
        summaries.append(sm)
    ...
    upstream_ok = s88.get("status") == "SOURCE_UNIVERSE_FILTERED_COMPONENT_RECONSTRUCTION_NOT_MATCHED_AUDIT_ONLY_LIVE_BLOCKED"
    inputs_ok = bool(inv["exists"].all()) if not inv.empty else False
    if full:
        status = "SOURCE_UNIVERSE_RULE_TUPLE_MEMBERSHIP_CANDIDATE_FOUND_AUDIT_ONLY_HUMAN_REVIEW_REQUIRED_LIVE_BLOCKED"
        next_step = "HUMAN_REVIEW_RULE_TUPLE_MEMBERSHIP_CANDIDATE"
    else:
        status = "SOURCE_UNIVERSE_RULE_TUPLE_MEMBERSHIP_NOT_MATCHED_AUDIT_ONLY_LIVE_BLOCKED"
        next_step = "REVIEW_TOP_LEDGER_GENERATION_AS_SELECTED_HISTORICAL_CLUSTER_SUMMARY_OR_DEFINE_NEW_POLICY"
    if not upstream_ok or not inputs_ok:
        status = "SOURCE_UNIVERSE_RULE_TUPLE_MEMBERSHIP_INPUT_REVIEW_REQUIRED_AUDIT_ONLY"
        next_step = "REVIEW_25C89_INPUTS"
    decision = pd.DataFrame([["upstream_25c88_ok", upstream_ok, True, "PASS" if upstream_ok else "FAIL"], ["inputs_present", inputs_ok, True, "PASS" if inputs_ok else "FAIL"], ["source_rule_rows", len(rules), ">0", "PASS" if len(rules)>0 else "FAIL"], ["top125_rows", len(top), 125, "PASS" if len(top)==125 else "FAIL"], ["full_rule_tuple_membership_reconstruction", full, True, "PASS" if full else "BLOCKED"], ["coreb_live_evaluator_allowed", False, False, "PASS"], ["a002_used", False, False, "PASS"]], columns=["decision_item", "observed", "required", "status"])
    blockers = pd.DataFrame([["B89-001", "rule_tuple_membership", "OPEN" if not full else "REVIEW", "HARD", "No complete match found" if not full else "Candidate requires human review"], ["B89-002", "CoreB live evaluator", "OPEN", "HARD", "Live remains blocked"], ["B89-003", "A002", "CLOSED_FOR_COREB_MAIN_PATH", "INFO", "A002 not used"]], columns=["blocker_id", "component", "status", "severity", "detail"])
    summary = {"created_utc": created, "step": STEP, "status": status, "audit_only": True, "upstream_25c88_ok": upstream_ok, "inputs_ok": inputs_ok, "source_rule_rows": int(len(rules)), "top125_rows": int(len(top)), "best_mode": str(best.iloc[0]["mode"]) if not best.empty else None, "best_same_count_exact": int(best.iloc[0]["same_count_exact"]) if not best.empty else 0, "best_source_rule_count_exact": int(best.iloc[0]["source_rule_count_exact"]) if not best.empty else 0, "full_match_found": full, "coreb_historical_sot_report_allowed": True, "coreb_live_evaluator_allowed": False, "final_signal_allowed": False, "a002_used": False, "source_recovery_approved": False, "external_actions": EXTERNAL_ACTIONS, "next_recommended_step": next_step}
    inv.to_csv(out/"25c89_input_inventory.csv", index=False, encoding="utf-8-sig")
    rules.to_csv(out/"25c89_source_rule_tuple_inventory.csv", index=False, encoding="utf-8-sig")
    summary_df.to_csv(out/"25c89_tuple_membership_reconstruction_summary.csv", index=False, encoding="utf-8-sig")
    detail_df.to_csv(out/"25c89_tuple_membership_reconstruction_rows.csv", index=False, encoding="utf-8-sig")
    best.to_csv(out/"25c89_best_candidate_matrix.csv", index=False, encoding="utf-8-sig")
    decision.to_csv(out/"25c89_decision_matrix.csv", index=False, encoding="utf-8-sig")
    blockers.to_csv(out/"25c89_blocker_matrix.csv", index=False, encoding="utf-8-sig")
    write_json(out/"25c89_summary.json", summary)
    report = "\n".join(["# GOLD V2 25C89 source-universe rule tuple membership audit-only report", "", f"Created UTC: {created}", f"Status: `{status}`", "", "## Tuple membership reconstruction summary", md(summary_df, 20), "", "## Decision matrix", md(decision), "", "## Blockers", md(blockers), "", "## Safety", "- audit_only: true", "- A002 not used", "- source recovery not approved", "- live/final/external actions remain OFF"])
    (out/"GOLD_V2_25C89_SOURCE_UNIVERSE_RULE_TUPLE_MEMBERSHIP_AUDIT_ONLY_REPORT.md").write_text(report, encoding="utf-8")
    zip_path = fx_outputs()/"gold_v2_25c89_source_universe_rule_tuple_membership_audit_only.zip"
