...
def write_stop(out: Path, status: str, input_audit: pd.DataFrame, diagnostic: pd.DataFrame, stop_rows: int) -> None:
    ...
    summary = {
        ...
        "status": status,
        "audit_only": True,
        "filter_attribution_executed": False,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    base44 = fx_outputs() / IN44
    base43 = fx_outputs() / IN43
    base36 = fx_outputs() / IN36
    base10 = fx_outputs() / IN10
    req = {
        "s44": base44 / "02_25c44_coreb_g1_right_only_damage_route_plan_summary.json",
        "rec44": base44 / "06_25c44_route_recommendation_matrix.csv",
        "gates44": base44 / "09_25c44_acceptance_gate_matrix.csv",
        "next44": base44 / "10_25c44_next_step_plan.csv",
        "class43": base43 / "04_25c43_right_only_driver_classification_matrix.csv",
        "bundles36": base36 / "04_25c36_adjusted_bundle_candidate_matrix.csv",
        "members36": base36 / "05_25c36_adjusted_bundle_membership.csv",
        "signals10": base10 / "04_25c10_filter_replay_signal_rows.csv",
    }
    ...
    rec44 = read_csv(req["rec44"])
    next44 = read_csv(req["next44"])
    contract = pd.DataFrame([
        ...
        {"contract_id": "C007", "check": "25C44 external/live flags false", "observed": "all false", "status": "PASS" if not any(as_bool(s44.get(k, True)) for k in ["coreb_live_evaluator_unblocked", "discord_notification_sent", "mt5_order_sent", "ai_api_called", "live_hook_executed", "final_signal_created"]) else "STOP"},
    ])
    ...
    if "filter" not in signals.columns:
        diag = pd.DataFrame([{"check": "25C10 signals include filter column", "observed": False, "status": "STOP"}])
        write_stop(out, STOP_ATTR, input_audit, diag, 1)
        return 2

    damage = class43[class43["driver_class"].astype(str).eq(DAMAGE_CLASS)].copy()
    damage = norm_key(damage)
    sig = signals[KEY + ["filter"]].drop_duplicates().copy()
    sig["filter"] = sig["filter"].astype(str)
    joined = damage.merge(sig, on=KEY, how="left")
    joined["filter_attributed"] = joined["filter"].astype(str).ne("")
    ...
    for _, b in bundles.iterrows():
        ...
        variant = bid + "_" + bname
        fs = members[members[id_col].astype(str).eq(bid)]["filter"].astype(str).tolist()
        for f in fs:
            excluded_rows.append({"variant": variant, "filter": f, "filter_excluded_by_variant": True})
    excluded = pd.DataFrame(excluded_rows)
    if excluded.empty:
        diag = pd.DataFrame([{"check": "25C36 excluded filter membership non-empty", "observed": False, "status": "STOP"}])
        write_stop(out, STOP_ATTR, input_audit, diag, 1)
        return 2

    attr = joined.merge(excluded, on=["variant", "filter"], how="left")
    ...
    by_filter = attr.groupby(["variant", "filter"], dropna=False).agg(
        damaged_key_rows=("entry_time", "count"),
        unique_damaged_keys=("entry_time", "nunique"),
        filter_excluded_by_variant=("filter_excluded_by_variant", "max"),
        cleanly_attributed_rows=("cleanly_attributed", "sum"),
    ).reset_index().sort_values(["variant", "damaged_key_rows"], ascending=[True, False])
    write_csv(out / "05_25c45_filter_damage_by_variant_matrix.csv", by_filter)

    variant_damage = attr.groupby(["variant"], dropna=False).agg(
        damage_rows=("entry_time", "count"),
        cleanly_attributed_rows=("cleanly_attributed", "sum"),
        attributed_rows=("filter_attributed", "sum"),
        excluded_filter_matched_rows=("filter_excluded_by_variant", "sum"),
    ).reset_index()
    variant_damage["clean_attribution_rate_pct"] = (variant_damage["cleanly_attributed_rows"] / variant_damage["damage_rows"].replace(0, pd.NA) * 100).round(2).fillna(0)
    write_csv(out / "06_25c45_variant_excluded_filter_damage_matrix.csv", variant_damage)

    retention = by_filter[by_filter["filter_excluded_by_variant"].astype(bool)].copy()
    retention["retention_priority"] = retention.groupby("variant")["damaged_key_rows"].rank(method="dense", ascending=False).astype(int)
    ...
    total_damage_rows = int(len(attr))
    clean_rows = int(attr["cleanly_attributed"].sum())
    unattributed_rows = int((~attr["filter_attributed"]).sum())
    not_excluded_rows = int((attr["filter_attributed"] & ~attr["filter_excluded_by_variant"]).sum())
    quality = pd.DataFrame([
        {"quality_id": "Q001", "check": "incremental damage rows loaded", "observed": total_damage_rows, "status": "PASS" if total_damage_rows > 0 else "STOP"},
        {"quality_id": "Q002", "check": "all rows have baseline filter attribution", "observed": unattributed_rows, "status": "PASS" if unattributed_rows == 0 else "STOP"},
        {"quality_id": "Q003", "check": "all attributed filters are excluded by their variant", "observed": not_excluded_rows, "status": "PASS" if not_excluded_rows == 0 else "STOP"},
        {"quality_id": "Q004", "check": "clean attribution rate", "observed": round((clean_rows / total_damage_rows * 100) if total_damage_rows else 0, 2), "status": "PASS" if clean_rows == total_damage_rows and total_damage_rows > 0 else "STOP"},
    ])
    write_csv(out / "08_25c45_attribution_quality_matrix.csv", quality)

    if bool((quality["status"] == "STOP").any()):
        write_stop(out, STOP_ATTR, input_audit, quality, int((quality["status"] == "STOP").sum()))
        return 2
    ...
    next_plan = pd.DataFrame([
        {"rank": 1, "next_step": NEXT_STEP, "allowed_now": True, "purpose": "design a retention-aware recovery plan using attributed damaging filters; no dry-run", "requires_human_acceptance_before_execution": False, "execution_allowed_in_25c45": False},
    ...
    summary = {
        ...
        "final_signal_created": False,
        "no_signal_discord_notify": False,
        "incremental_damage_rows": total_damage_rows,
        "cleanly_attributed_rows": clean_rows,
        "unattributed_rows": unattributed_rows,
        "attributed_not_excluded_rows": not_excluded_rows,
    ...
    report = "\n".join([
        ...
        "## Scope",
        "",
        "This step attributes incremental damage keys to baseline replay filters only. It does not run a dry-run, search variants, change conditions, approve variants, or unblock live behavior.",
        "",
        "## 25C44 contract audit",
        "",
        md_table(contract),
        "",
        "## Attribution quality",
        ...
        md_table(quality),
        "",
        "## Filter damage by variant",
        "",
        md_table(by_filter),
        "",
        "## Variant attribution summary",
        "",
        md_table(variant_damage),
        "",
        ...
        *[f"{i+1:02d}. {x}" for i, x in enumerate(reqdf[reqdf["section"].eq("必要・見るファイル")]["item"].tolist())],
        ...
        "## Safety",
        "",
        "- 25C45 is attribution-only and does not approve any variant.",
        "- CoreB live evaluator, Discord, MT5, AI API, live hook, and final signal remain blocked.",
        "- Future dry-run remains blocked until retention-aware plan and separate explicit human acceptance.",
