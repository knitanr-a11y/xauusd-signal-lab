...
def inventory(paths: dict[str, Path | None]) -> pd.DataFrame:
    ...
    for n, p in paths.items():
        ...
        if p and p.exists():
            r.update({"bytes": p.stat().st_size, "sha256": sha256_file(p)})
        rows.append(r)
...
def is_time(c: str) -> bool:
    return norm(c) in TIME or "time" in norm(c)


def is_month_fold(c: str) -> bool:
    return norm(c) in MONTH_FOLD or "month" in norm(c) or "fold" in norm(c)


def is_id(c: str) -> bool:
    low = norm(c)
    return low in IDLIKE or low.endswith("_id") or "candidate" in low or "origin" in low or "variant" in low


def is_cluster(c: str) -> bool:
    return "cluster" in norm(c)
...
def variant_cols(component: str, cols: list[str]) -> dict[str, list[str]]:
    ...
    coarse = feature[:]
    return {
        "full_25c110_signature": full,
        "no_time_columns": no_time,
        "no_time_or_id_columns": no_time_id,
        "feature_only_no_time_id_cluster_month_fold": feature,
        "coarse_feature_family_only": coarse,
    }
...
def coarse_df(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    ...
    for c in cols:
        if pd.api.types.is_numeric_dtype(out[c]):
            ...
            # Stable coarse bins; avoids qcut dependence on selected/universe distributions.
...
def add_audit_metadata(df: pd.DataFrame, component: str, variant: str) -> pd.DataFrame:
    """Add metadata without colliding with signature columns such as `component`."""
    out = df.copy()
    out.insert(0, "audit_selected_row_number_1based", out.index + 1)
    out.insert(0, "audit_signature_variant", variant)
    out.insert(0, "audit_component", component)
    return out
...
def contrast(sel: pd.DataFrame, uni: pd.DataFrame, cols: list[str], component: str, variant: str) -> tuple[dict[str, Any], pd.DataFrame]:
    if not cols:
        sm = {
            ...
            "selected_unique_matches": 0,
            "selected_ambiguous_matches": 0,
            "selected_no_matches": len(sel),
            "ambiguous_ratio": 1.0 if len(sel) else 0,
            "no_match_ratio": 1.0 if len(sel) else 0,
            "selected_signature_groups": 0,
            "selected_groups_multi_selected": 0,
        }
    ...
    no = int((joined["universe_match_count"] == 0).sum())
    uniq = int((joined["universe_match_count"] == 1).sum())
    amb = int((joined["universe_match_count"] > 1).sum())
    sg = sdata.groupby(cols, dropna=False).size().reset_index(name="selected_group_rows")
    multi = int((sg["selected_group_rows"] > 1).sum())
    ambrows = add_audit_metadata(joined[joined["universe_match_count"] != 1], component, variant)
    sm = {
        "component": component,
        "signature_variant": variant,
        "signature_columns": len(cols),
        "selected_rows": len(sel),
        "universe_rows": len(uni),
        "selected_unique_matches": uniq,
        "selected_ambiguous_matches": amb,
        "selected_no_matches": no,
        "ambiguous_ratio": float(amb / len(sel)) if len(sel) else 0,
        "no_match_ratio": float(no / len(sel)) if len(sel) else 0,
        "selected_signature_groups": len(sg),
        "selected_groups_multi_selected": multi,
    }
...
def main() -> int:
    ...
    inv = inventory(paths)
    s110 = read_json(paths["25c110_summary.json"])
    pairs = read_csv(paths["25c110_pair_inventory.csv"])
    sigcols = read_csv(paths["25c110_entry_signature_columns.csv"])
    inputs_ok = bool(inv["exists"].all()) if not inv.empty else False
    upstream_ok = s110.get("status") == EXPECTED_STATUS
    ...
    for _, p in pairs.iterrows():
        ...
        selp = resolve(str(p.get("selected_path", "")))
        unip = resolve(str(p.get("universe_path", "")))
        ...
        for v, vc in variants.items():
            sm, amb = contrast(sel, uni, vc, comp, v)
            summaries.append(sm)
            if not amb.empty:
                ambs.append(amb)
            for c in vc:
                sig_rows.append({"component": comp, "signature_variant": v, "column": c, "is_time": is_time(c), "is_id": is_id(c), "is_month_fold": is_month_fold(c), "is_cluster": is_cluster(c)})

    pair_df = pd.DataFrame(pair_rows)
    sum_df = pd.DataFrame(summaries)
    sig_df = pd.DataFrame(sig_rows)
    amb_df = pd.concat(ambs, ignore_index=True) if ambs else pd.DataFrame()
    ...
    feature_rows = critical[critical["signature_variant"].eq("feature_only_no_time_id_cluster_month_fold")] if not critical.empty else pd.DataFrame()
    ...
    if not (inputs_ok and upstream_ok and pairs_ok):
        status = "COREA_MEDIUM_TIME_ID_ABLATION_INPUT_REVIEW_REQUIRED_AUDIT_ONLY"
    elif ambiguity:
        status = "COREA_MEDIUM_TIME_ID_ABLATION_AMBIGUITY_FOUND_AUDIT_ONLY_LIVE_BLOCKED"
    elif feature_unique:
        status = "COREA_MEDIUM_FEATURE_ONLY_UNIQUENESS_CANDIDATE_AUDIT_ONLY_HUMAN_REVIEW_REQUIRED_LIVE_BLOCKED"
    else:
        status = "COREA_MEDIUM_TIME_ID_ABLATION_AMBIGUITY_FOUND_AUDIT_ONLY_LIVE_BLOCKED"
    ...
    summary = {
        ...
        "upstream_25c110_ok": upstream_ok,
        "inputs_present": inputs_ok,
        "pairs_readable": pairs_ok,
        "ablation_ambiguity_or_no_match": ambiguity,
    ...
    report = "\n".join([
        "# GOLD V2 25C111 CoreA/MEDIUM time-ID ablation audit-only report",
        "",
        ...
        md(sum_df),
        "",
        "## Ablation signature columns",
        md(sig_df),
        "",
        "## Ambiguous/no-match rows",
        md(amb_df),
        "",
        "## Blockers",
        md(blockers),
