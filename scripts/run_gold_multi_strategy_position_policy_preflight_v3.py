#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Dry-run preflight v3 for future GOLD multi-strategy position policy.

v3 adds mock-position support on top of v2.

Purpose:
- Validate the future strategy-aware policy without changing the real sender.
- Test same_strategy and total_position_cap behavior without opening real demo positions.
...
Modes:
1. Real MT5 mode, default:
   - Initializes MT5.
   - Checks expected account / demo guard.
   - Reads current open positions.

2. Mock positions mode:
   - Use --mock-positions-csv.
   - Does not initialize MT5 by default.
   - Reads positions from CSV.
   - Still evaluates the exact same policy rows.
...
4. total open positions >= max_total_positions => BLOCK
5. requested lot > max_lot_per_order => BLOCK
6. otherwise => ALLOW
"""
...
SCHEMA_VERSION = "gold_multi_strategy_position_policy_preflight_v3"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Dry-run preflight v3 with optional mock positions. No order_send/order_check.")
    p.add_argument("--input-csv", type=Path, default=DEFAULT_INPUT_CSV)
    p.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    p.add_argument("--output-csv", type=Path, default=None)
    p.add_argument("--output-json", type=Path, default=None)
    p.add_argument("--positions-csv", type=Path, default=None)
    p.add_argument("--order-ledger-csv", type=Path, default=DEFAULT_ORDER_LEDGER_CSV)
    p.add_argument("--symbol", type=str, default=None, help="Broker symbol override / snapshot focus, e.g. GOLD#")
    ...
    p.add_argument("--block-unknown-same-symbol-strategy", action="store_true")
    p.add_argument(
        "--mock-positions-csv",
        type=Path,
        default=None,
        help="If provided, use this CSV as the open-position snapshot instead of MT5 positions.",
    )
    p.add_argument(
        "--mock-account-login",
        type=int,
        default=75539039,
        help="Account login recorded in JSON during mock mode.",
    )
    p.add_argument(
        "--mock-account-server",
        type=str,
        default="MOCK-MT5",
        help="Account server recorded in JSON during mock mode.",
    )
    return p.parse_args()
...
def summarize_positions(positions: list[dict[str, Any]], known_strategy_keys: Iterable[str] = ()) -> str:
    ...
    for p in positions:
        ...
        parts.append(
            "ticket={ticket},symbol={symbol},direction={direction},type={type},volume={volume},price_open={price_open},sl={sl},tp={tp},magic={magic},comment={comment},external_id={external_id},detected_strategy={detected}".format(
                ticket=p.get("ticket"),
                symbol=p.get("symbol"),
...
def positions_dataframe(positions: list[dict[str, Any]], snapshot_time_utc: str, position_source: str) -> pd.DataFrame:
...
def read_mock_positions(path: Path) -> list[dict[str, Any]]:
    df = read_csv(path)
    if df.empty:
        return []
    rows: list[dict[str, Any]] = []
    for _, row in df.iterrows():
        d = {str(k): row.get(k) for k in df.columns}
        rows.append(d)
    return rows


def get_real_mt5_positions(args: argparse.Namespace) -> tuple[bool, int, dict[str, Any], list[dict[str, Any]]]:
    report: dict[str, Any] = {
    ...
    try:
        ...
        add_position_summary(report, positions, args.symbol)
...
def get_mock_positions(args: argparse.Namespace) -> tuple[bool, int, dict[str, Any], list[dict[str, Any]]]:
    assert args.mock_positions_csv is not None
    report: dict[str, Any] = {
        "position_source": "MOCK_CSV",
        "mock_positions_csv": str(args.mock_positions_csv),
    ...
    if not args.mock_positions_csv.exists():
        report["fatal_error"] = "MOCK_POSITIONS_CSV_NOT_FOUND"
        return False, 20, report, []
    try:
        positions = read_mock_positions(args.mock_positions_csv)
    except Exception as e:
        report["fatal_error"] = "MOCK_POSITIONS_READ_ERROR"
        report["mock_positions_read_error"] = repr(e)
        return False, 21, report, []
    add_position_summary(report, positions, args.symbol)
    return True, 0, report, positions


def add_position_summary(report: dict[str, Any], positions: list[dict[str, Any]], symbol: str | None) -> None:
    report["existing_total_positions"] = int(len(positions))
    report["existing_positions_detail"] = summarize_positions(positions)
    if symbol:
        symbol_positions = same_symbol_positions(positions, symbol)
...
def build_preflight_row(
    *,
    ...
    if same_strategy_positions:
        same_strategy_reasons.append(f"existing same-strategy position(s) detected: count={len(same_strategy_positions)}; strategy_key={req_strategy_key}; detected={detected_strategy_keys}")
    ...
    opposite_reason = f"existing same-symbol opposite direction position(s): requested={requested_direction}; existing_directions={symbol_directions}" if opposite_blocked else ""

    total_position_cap_blocked = len(all_positions) >= int(args.max_total_positions)
    total_position_cap_reason = f"total open positions cap reached: existing_total_positions={len(all_positions)}; max_total_positions={int(args.max_total_positions)}" if total_position_cap_blocked else ""

    per_order_lot_blocked = bool(requested_lot is not None and requested_lot > float(args.max_lot_per_order) + 1e-12)
    per_order_lot_reason = f"requested lot exceeds per-order cap: requested_lot={requested_lot}; max_lot_per_order={float(args.max_lot_per_order)}" if per_order_lot_blocked else ""

    input_validation_blocked = bool(input_errors)
    warnings: list[str] = []
    if symbol_positions and not detected_strategy_keys:
        warnings.append(
            "No strategy key was detectable from existing same-symbol position comments/external_id. "
            "same_strategy_block can only be exact-match until registry/comment metadata exists."
        )
...
def main() -> int:
    ...
    position_source = "MOCK_CSV" if args.mock_positions_csv is not None else "MT5_REAL"
    if args.mock_positions_csv is not None:
        positions_ok, positions_rc, position_report, all_positions = get_mock_positions(args)
    else:
        positions_ok, positions_rc, position_report, all_positions = get_real_mt5_positions(args)

    write_csv(positions_dataframe(all_positions, now, position_source), positions_csv)
    ...
    if not positions_ok:
        ...
        summary = {
            ...
            "preflight_ok": False,
            "policy_name": POLICY_NAME,
            "reason": position_report.get("fatal_error", "POSITION_SNAPSHOT_FAILED"),
            "position_source": position_source,
            "input_status": input_status,
            "input_csv": str(args.input_csv),
            "input_exists": bool(input_exists),
            "mock_positions_csv": str(args.mock_positions_csv) if args.mock_positions_csv else "",
        ...
        print("run_gold_multi_strategy_position_policy_preflight_v3")
        print(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True, default=str))
        if position_source == "MT5_REAL" and mt5 is not None:
            mt5.shutdown()
        return positions_rc
    ...
    no_payload_snapshot_ok = src.empty and positions_ok
    ...
    if not out.empty:
        ...
        print(f"[INFO] no payload rows; position snapshot written: {positions_csv}")
