...
READY = "GOLD_V3_105_INDEPENDENT_HIGH_VOL_PROXY_READY_AUDIT_ONLY"
BLOCKED = "GOLD_V3_105_INDEPENDENT_HIGH_VOL_PROXY_BLOCKED_AUDIT_ONLY"
...
def find_files_dir() -> Path:
    ...
    for d in [Path.cwd(), root, root.parent, root.parent.parent, root / "Files", root.parent / "Files"]:
        d = d.resolve()
        if (d / "goldsharp_m15.csv").exists() and (d / "goldsharp_m5.csv").exists() and (d / "FX_OUTPUTS" / "gold_v3").exists():
            return d
...
def metrics(df: pd.DataFrame, by: list[str]) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(columns=by + ["trades", "wins", "losses", "win_rate", "gross_profit", "gross_loss", "profit_factor", "sum_result_usd", "avg_result_usd"])
    g = []
    for key, x in df.groupby(by, dropna=False):
        if not isinstance(key, tuple):
            key = (key,)
        gp = float(x.loc[x.result_usd > 0, "result_usd"].sum())
        gl = float(-x.loc[x.result_usd < 0, "result_usd"].sum())
        pf = gp / gl if gl > 0 else (math.inf if gp > 0 else 0.0)
        row = {k: v for k, v in zip(by, key)}
        row.update(trades=int(len(x)), wins=int((x.result_usd > 0).sum()), losses=int((x.result_usd < 0).sum()), win_rate=float((x.result_usd > 0).mean()) if len(x) else 0.0, gross_profit=gp, gross_loss=gl, profit_factor=pf, sum_result_usd=float(x.result_usd.sum()), avg_result_usd=float(x.result_usd.mean()) if len(x) else 0.0)
        g.append(row)
    return pd.DataFrame(g).sort_values(["profit_factor", "trades"], ascending=[False, False])


def evaluate(opps: pd.DataFrame, m5: pd.DataFrame) -> pd.DataFrame:
    if opps.empty:
        return opps
    m5 = m5.sort_values("time").reset_index(drop=True)
    times = m5.time.values
    lo = m5.low.to_numpy(float); hi = m5.high.to_numpy(float); cl = m5.close.to_numpy(float)
    out = []
    for _, r in opps.iterrows():
        ...
        end = et + pd.Timedelta(minutes=15*h)
        si = np.searchsorted(times, np.datetime64(et), side="right")
        ei = np.searchsorted(times, np.datetime64(end), side="right")
        if ei <= si or ei >= len(m5) or m5.time.iloc[ei-1] < end - pd.Timedelta(minutes=5):
            continue
        tp_px = entry + tp; sl_px = entry - sl
        sl_hit = np.where(lo[si:ei] <= sl_px)[0]
        tp_hit = np.where(hi[si:ei] >= tp_px)[0]
        ...
        if fs is not None and (ft is None or fs <= ft):
            ...
            ai = ei - 1; ep = float(cl[ai]); res = (m5.time.iloc[ai], ep, "TIMEOUT", ep-entry)
        d = r.to_dict(); d.update(exit_dt=res[0], exit_price=res[1], exit_reason=res[2], result_usd=float(res[3]), is_win=res[3] > 0, is_loss=res[3] < 0)
        out.append(d)
...
def main() -> int:
    ...
    out = base / "105c"; out.mkdir(parents=True, exist_ok=True)
    ...
    if not blockers:
        ...
        m15, m5 = st45.prepare(src, "closed", 60, 0.70)
        q = read_csv(p50q)
        if not q.empty:
        ...
        for _, r in win.iterrows():
            for pid, tp, sl, h in HV_PROFILES:
                rows.append({"entry_dt": r.time, "jst_dt": r.jst_dt, "entry_month": r.entry_month, "profile_id": pid, "candidate_label": f"INDEPENDENT_HV__{pid}", "tp_usd": tp, "sl_usd": sl, "horizon_m15": h, "entry_price": r.close, "h4_ret4": r.h4_ret4, "m15_atr28": r.m15_atr28, "m15_atr28_q": r.m15_atr28_q, "is_high_vol": r.is_high_vol, "jst_hour": r.jst_hour, "jst_weekday": r.jst_weekday})
        opps = pd.DataFrame(rows)
        if not opps.empty:
            opps["h4_bucket"] = pd.cut(pd.to_numeric(opps["h4_ret4"], errors="coerce"), bins=[-999, -0.02, -0.01, 0, 0.01, 0.02, 999], labels=["lt_-2pct", "-2_to_-1pct", "-1_to_0pct", "0_to_1pct", "1_to_2pct", "gt_2pct"])
        ev = evaluate(opps, m5)
        if not ev.empty:
            ev["h4_bucket"] = pd.cut(pd.to_numeric(ev["h4_ret4"], errors="coerce"), bins=[-999, -0.02, -0.01, 0, 0.01, 0.02, 999], labels=["lt_-2pct", "-2_to_-1pct", "-1_to_0pct", "0_to_1pct", "1_to_2pct", "gt_2pct"])
        checks.append(ok("independent_hv_opportunities_positive", len(opps) > 0, len(opps), ">0", "WARN"))
    for c in checks:
    ...
    prof = metrics(ev, ["profile_id"]) if not ev.empty else pd.DataFrame()
    weekday = metrics(ev, ["jst_weekday"]) if not ev.empty else pd.DataFrame()
    hour = metrics(ev, ["jst_hour"]) if not ev.empty else pd.DataFrame()
    h4m = metrics(ev, ["h4_bucket"]) if not ev.empty else pd.DataFrame()
    opps.to_csv(out / "independent_high_vol_proxy_opportunities.csv", index=False, encoding="utf-8-sig")
    ev.to_csv(out / "independent_high_vol_proxy_evaluated_trades.csv", index=False, encoding="utf-8-sig")
    prof.to_csv(out / "profile_metrics.csv", index=False, encoding="utf-8-sig")
    weekday.to_csv(out / "weekday_metrics.csv", index=False, encoding="utf-8-sig")
    hour.to_csv(out / "hour_metrics.csv", index=False, encoding="utf-8-sig")
    h4m.to_csv(out / "h4_bucket_metrics.csv", index=False, encoding="utf-8-sig")
