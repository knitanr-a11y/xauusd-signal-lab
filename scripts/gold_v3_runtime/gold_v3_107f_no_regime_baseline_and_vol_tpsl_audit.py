...
def side_gate(df,side,win,min_n,pf_thr):
    x=df[df.proxy_side==side].sort_values(['entry_dt','priority','candidate_label']).copy()
    comp=x[x.exit_dt.notna() & x.result_usd.notna()].sort_values(['exit_dt','entry_dt','priority','candidate_label']).to_dict('records')
...
def dual_global(df,win,min_n,pf_thr,margin):
    ...
    for t,g in x.groupby('entry_dt',sort=True):
        ...
        lp=pf(list(hist['LONG'])) if len(hist['LONG'])>=min_n else 0.0
        sp=pf(list(hist['SHORT'])) if len(hist['SHORT'])>=min_n else 0.0
        side='NO_TRADE'
...
def load_m5(p):
    ...
    out=df[[t,h,l]].copy(); out.columns=['time','high','low']
    out['time']=pd.to_datetime(out.time,errors='coerce'); out['high']=pd.to_numeric(out.high,errors='coerce'); out['low']=pd.to_numeric(out.low,errors='coerce')
...
def main():
    ...
    ledger=base/'FX_OUTPUTS'/'gold_v3'/'107c'/'gold_v3_107_long_short_proxy_ledger.csv'
    blockers=[]; vals=[]; findings=[]; outputs=[]; rows=[]; selected_parts=[]; vol_rows=[]
    ...
    if not blockers:
        ...
        for side,win,mn,pft in itertools.product(['LONG','SHORT'],[20,30,50],[10,20],[1.0,1.15,1.3]):
            ...
            if not sel.empty: selected_parts.append(sel.assign(policy=pol,window=win,min_history=mn,pf_threshold=pft))
        for win,mn,pft,mar in itertools.product([20,30,50],[10,20],[1.0,1.15,1.3],[1.0,1.15,1.3]):
            ...
            if not sel.empty: selected_parts.append(sel.assign(policy='no_regime_dual_edge_global',window=win,min_history=mn,pf_threshold=pft,side_margin=mar))
        ...
        if m5p is None:
            findings.append('vol_tpsl_skipped: exact M5 CSV not found; no broad scan used; TP floor is 5.0 USD; SL has no fixed 5.0 USD floor')
            vol=pd.DataFrame([dict(status='SKIPPED',reason='exact M5 CSV not found',min_tp_usd=5.0,sl_rule='SL=TP/RR; no fixed 5 USD floor')])
        else:
            m5=load_m5(m5p); base_sel=allsel.head(300).copy() if len(allsel) else df.head(300).copy()
            for tm,rr in itertools.product([0.5,0.75,1.0,1.25],[1.5,2.0,2.5,3.0]):
                x=base_sel.copy(); atr=pd.to_numeric(x.get('m15_atr28',np.nan),errors='coerce').fillna(0)
                x['dyn_tp_usd']=np.maximum(5.0,atr*tm); x['dyn_sl_usd']=x['dyn_tp_usd']/rr
                x['result_usd']=[eval_dyn(r,m5,r.dyn_tp_usd,r.dyn_sl_usd) for _,r in x.iterrows()]
                mm=metric(x); vol_rows.append(dict(tp_mult=tm,rr=rr,min_tp_usd=5.0,sl_rule='TP/RR_no_fixed_5_usd_floor',m5_source=str(m5p),**mm,score=score(mm)))
