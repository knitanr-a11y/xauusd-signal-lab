...
STEP = 'GOLD_V3_185_PRIMARY_ABC_PORTFOLIO_FREEZE_AUDIT_ONLY'
...
PRIMARY_CANDIDATES = [
    {
        ...
        'role': 'PRIMARY',
        'priority': 1,
        'description': 'Primary precision candidate. Strongest PF, lower frequency.',
    ...
    },
    {
        ...
        'role': 'PRIMARY',
        'priority': 3,
        'description': 'Primary high-frequency candidate. Most trades, rougher monthly profile.',
    ...
    },
    {
        ...
        'role': 'PRIMARY',
        'priority': 2,
        'description': 'Primary balanced candidate with Stage184 h1_atr14 cap. Main balanced route.',
...
def progress(msg: str) -> None:
    print(f'[185 progress] {msg}', flush=True)
...
def eval_candidate(data: pd.DataFrame, m5: pd.DataFrame, candidate: dict[str, Any], cost_points: float) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame]:
    ...
    dedup['priority'] = int(candidate['priority'])
...
def priority_unique_portfolio(trades: pd.DataFrame) -> pd.DataFrame:
    if trades.empty:
        return pd.DataFrame()
    t = trades.copy()
    t['entry_dt_str'] = pd.to_datetime(t['entry_dt']).astype(str)
    t = t.sort_values(['entry_dt_str', 'priority', 'candidate_id'])
    return t.groupby('entry_dt_str', as_index=False).head(1).copy()
...
def main() -> int:
    ...
    if not source_diag.empty:
        save(source_diag, out / 'gold_v3_185_source_coverage.csv')
    ...
    priority_portfolio = pd.DataFrame()
    priority_monthly = pd.DataFrame()
    ...
    if not blockers:
        ...
        for c in PRIMARY_CANDIDATES:
            ...
            s, entries, dedup = eval_candidate(data, frames['m5'], c, float(args.cost_points))
            summaries.append(s)
            entry_counts.append({'candidate_id': c['candidate_id'], 'entry_rows_before_dedup': int(len(entries))})
            if s.get('status') != 'OK':
        ...
        priority_portfolio = priority_unique_portfolio(trades_all)
        priority_monthly = monthly_table(priority_portfolio) if not priority_portfolio.empty else pd.DataFrame()
        save(pd.DataFrame(PRIMARY_CANDIDATES), out / 'gold_v3_185_primary_abc_candidates.csv')
        (out / 'gold_v3_185_primary_abc_candidates.json').write_text(json.dumps({'audit_only': True, 'primary_candidates': PRIMARY_CANDIDATES}, ensure_ascii=False, indent=2), encoding='utf-8')
        save(cand_summary, out / 'gold_v3_185_candidate_summary.csv')
        save(trades_all, out / 'gold_v3_185_trades_by_candidate.csv')
        save(monthly_all, out / 'gold_v3_185_monthly_by_candidate.csv')
        save(overlaps, out / 'gold_v3_185_overlap_entry_timestamps.csv')
        save(overlap_dist, out / 'gold_v3_185_overlap_distribution.csv')
        save(priority_portfolio, out / 'gold_v3_185_priority_unique_portfolio_trades.csv')
        save(priority_monthly, out / 'gold_v3_185_priority_unique_portfolio_monthly.csv')

    ready = len(blockers) == 0
    priority_metric = metric('priority_full', priority_portfolio['pnl_net'] if not priority_portfolio.empty else np.array([]))
    summary = {
        'step': STEP,
        'status': 'READY' if ready else 'BLOCKED',
        'ready': ready,
        'decision': 'STAGE185_PRIMARY_ABC_PORTFOLIO_FREEZE_READY_AUDIT_ONLY' if ready else 'STAGE185_BLOCKED',
        ...
        'priority_portfolio_trades': int(len(priority_portfolio)) if not priority_portfolio.empty else 0,
        **priority_metric,
    ...
    (out / 'gold_v3_185_summary.json').write_text(json.dumps({**summary, 'blockers': blockers}, ensure_ascii=False, indent=2), encoding='utf-8')
    save(pd.DataFrame([summary]), out / 'gold_v3_185_decision.csv')
    ...
    cand_cols = ['candidate_id', 'role', 'priority', 'direction', 'tp', 'sl', 'horizon_m5', 'entry_rows_before_dedup', 'dedup_n', 'train_n', 'train_pf', 'test_n', 'test_pf', 'full_n', 'full_pf', 'full_wr_pct', 'recent3m_n', 'recent3m_pf', 'full_neg_months', 'worst_month', 'worst_month_sum', 'tp_hits', 'sl_hits', 'horizon_exits', 'beats_old_pf_2_237', 'rule']
    cand_show = cand_summary[[c for c in cand_cols if c in cand_summary.columns]].sort_values('priority') if not cand_summary.empty else pd.DataFrame()
    monthly_show = monthly_all.sort_values(['candidate_id', 'month']) if not monthly_all.empty else pd.DataFrame()

    lines = ['GOLD V3 185 PASTE_ME_PRIMARY_ABC_PORTFOLIO_FREEZE_AUDIT']
    ...
    lines += ['', 'PRIORITY_UNIQUE_PORTFOLIO_MONTHLY', show(priority_monthly, 40)]
    lines += ['', 'DATA_COVERAGE', source_diag.to_string(index=False) if not source_diag.empty else 'NO_DATA_COVERAGE']
    lines += [
        '',
        'INTERPRETATION',
        'Stage185 is audit-only. It freezes A, B, and C as PRIMARY candidates for review. C uses the Stage184 h1_atr14<=60 variant. This does not enable live signal, payload, Discord, MT5 order, AI API, live hook, or autotrade.',
        'The overlap tables show how often multiple primary candidates trigger at the same entry timestamp. Priority unique portfolio is only an audit view for duplicate timestamp review; it is not live execution approval.',
    ]
