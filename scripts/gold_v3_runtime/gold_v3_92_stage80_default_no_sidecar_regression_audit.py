#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GOLD V3 92 Stage80 default no-sidecar regression audit-only."""
...
STEP = "GOLD_V3_92_STAGE80_DEFAULT_NO_SIDECAR_REGRESSION_AUDIT_ONLY"
READY_STATUS = "GOLD_V3_92_STAGE80_DEFAULT_NO_SIDECAR_REGRESSION_READY_AUDIT_ONLY"
BLOCKED_STATUS = "GOLD_V3_92_STAGE80_DEFAULT_NO_SIDECAR_REGRESSION_BLOCKED_AUDIT_ONLY"
...
def main() -> int:
    ...
    out = Path(a.output_dir).expanduser().resolve() if a.output_dir else base / "92c"
    ...
    durable_ledger_append_enabled = bool(j80.get("durable_ledger_append_enabled", False))
    ...
    val.extend([
        ...
        ok("durable_ledger_append_enabled_false", durable_ledger_append_enabled is False, durable_ledger_append_enabled, False),
    ...
    summary = {
        ...
        "live_ready": False,
        "stage80_default_no_sidecar_regression_ready": status == READY_STATUS,
        ...
        "next": "regular Stage80 BAT remains normal monitoring; Stage91 BAT is explicit sidecar test only",
    }
    write_text(out / "summary.json", json.dumps(summary, ensure_ascii=False, indent=2))

    paste = [
        "GOLD V3 92 PASTE_ME_STAGE80_DEFAULT_NO_SIDECAR_REGRESSION_SUMMARY",
        f"status: {status}",
        "stage80_default_no_sidecar_regression_ready: " + str(status == READY_STATUS).lower(),
        "live_ready: false",
        "contract_mutated: false",
        "manual_candidate_demotion_or_removal: false",
        "open_asof_allowed: false",
    ...
    report = f"""# GOLD V3 92 Stage80 default no-sidecar regression audit-only report
...
Default Stage80 behavior remains no-sidecar if READY.
"""
