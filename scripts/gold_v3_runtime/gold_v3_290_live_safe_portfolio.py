...
def load_runtime_state(path,latest_close):
    ...
    state={"initialized_at_utc":now(),"last_processed_planned_entry_dt":str(pd.Timestamp(latest_close)),"bootstrap_mode":"LATEST_ONLY"}
    write_json(path,state); return state,True
...
def main():
    ...
    runtime_state["last_processed_planned_entry_dt"]=str(latest_close); runtime_state["updated_at_utc"]=now(); runtime_state["bootstrap_state_start"]=a.bootstrap_state_start; write_json(state_path,runtime_state)
    status=READY if len(final) else NO_SIGNAL; write_json(out/"gold_v3_290_summary.json",{"status":status,"live_signal_ready":True,"created_at_utc":now(),"latest_m1_close":str(latest_close),"new_intent_count":int(len(intents)),"new_decision_count":int(len(cycle)),"final_signal_count":int(len(final)),"model_meta":meta,"mt5_order_enabled":False,"discord_enabled":False,"shadow_exit_simulation_used":False}); return 0
