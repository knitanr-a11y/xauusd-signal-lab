...
from gold_v3_290_ledger import empty_ledger,import_bootstrap,load_ledger
from gold_v3_290_readiness_gate import check_readiness
from gold_v3_290_updates import apply_updates,load_updates

BLOCKED="GOLD_V3_290_LIVE_SAFE_PORTFOLIO_SIGNAL_BLOCKED"
READY="GOLD_V3_290_LIVE_SAFE_PORTFOLIO_SIGNAL_READY"
NO_SIGNAL="GOLD_V3_290_LIVE_SAFE_PORTFOLIO_NO_SIGNAL"

def now(): return datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00","Z")

def args():
    p=argparse.ArgumentParser(); p.add_argument("--candle-dir",required=True); p.add_argument("--output-dir",default=""); p.add_argument("--bootstrap-ledger",required=True); p.add_argument("--authorization",required=True); p.add_argument("--lookback-hours",type=int,default=96); p.add_argument("--max-lag-seconds",type=int,default=120); return p.parse_args()
...
def main():
    ...
    state_path=out/"gold_v3_290_runtime_state.json"; runtime_state,boot=load_runtime_state(state_path,latest_close)
    ...
    if len(intents):
        intents["planned_entry_dt"]=pd.to_datetime(intents.planned_entry_dt,errors="coerce")
        intents=intents[intents.planned_entry_dt>watermark].copy()
        intents["intent_lag_seconds"]=(latest_close-intents.planned_entry_dt).dt.total_seconds()
    ...
    bootstrap=import_bootstrap(bootstrap_path); cycle,ledger=evaluate_intents(intents,ledger,bootstrap,a.max_lag_seconds)
    decisions=pd.concat([old_decisions,cycle],ignore_index=True,sort=False) if len(cycle) else old_decisions; write_csv(decision_path,decisions); write_csv(ledger_path,ledger)
    ...
    runtime_state["last_processed_planned_entry_dt"]=str(latest_close); runtime_state["updated_at_utc"]=now(); write_json(state_path,runtime_state)
    status=READY if len(final) else NO_SIGNAL; summary={"status":status,"live_signal_ready":True,"created_at_utc":now(),"latest_m1_close":str(latest_close),"new_intent_count":int(len(intents)),"new_decision_count":int(len(cycle)),"final_signal_count":int(len(final)),"model_meta":meta,"mt5_order_enabled":False,"discord_enabled":False,"shadow_exit_simulation_used":False}; write_json(out/"gold_v3_290_summary.json",summary); return 0
