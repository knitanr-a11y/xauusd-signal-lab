...
def main()->int:
    ...
    d1_rank=pd.DataFrame(); h1_rank=pd.DataFrame(); exact_rows=[]; current_inv=[]
    if not blockers:
        ...
        # H1 exhaustive range: range/tr/body over h1/d1 denominators
        ...
        # D1 dist exhaustive: source price - ref / denominator, including h1/m15 sources, d1 refs, d1/h1 denominators. Sample stage, then full top candidates.
        src_prices={**price_refs(m15,'m15'), **price_refs(h1,'h1')}
        ref_prices={**price_refs(d1,'d1')}
        # add D1 moving refs likely entry-available when using closed D1
        ...
        samp=pd.DataFrame(sample_rows).sort_values(['mae','p95_abs','max_abs']).head(TOP_FULL); save(samp,out/'gold_v3_176d_d1_dist_sample_top120.csv')
        # full metrics for sample winners only
        full=[]
        for _,r in samp.iterrows():
            sn, rn, dn, sh = r.src, r.ref, r.denom, int(r.shift)
        ...
        # current policy inventory: what columns exist and whether current can be ordered from ledger-only fields
