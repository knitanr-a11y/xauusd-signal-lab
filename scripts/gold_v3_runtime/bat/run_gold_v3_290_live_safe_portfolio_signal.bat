...
set "BASE_HISTORY=%OUT_DIR%\base_resolved_health.csv"
...
python "%RUNTIME%\gold_v3_290_build_base_history.py" --stage67 "%FILES_DIR%\FX_OUTPUTS\gold_v3\67_health_gate_rehydration_audit_only\gold_v3_67_health_gate_event_ledger.csv" --stage53 "%FILES_DIR%\FX_OUTPUTS\gold_v3\53_pending_to_closed_shadow_trade_adjudication_audit_only\gold_v3_53_closed_shadow_trade_ledger.csv" --output "%BASE_HISTORY%"
...
if errorlevel 1 (rmdir "%LOCK_DIR%"& exit /b 7)
python "%RUNTIME%\gold_v3_290_live_safe_portfolio.py" --candle-dir "%FILES_DIR%" --output-dir "%OUT_DIR%" --bootstrap-ledger "%GOLD_V3_SAFE_PORTFOLIO_LEDGER%" --base-resolved-health-ledger "%BASE_HISTORY%" --exact-parity-report "%PARITY_DIR%\gold_v3_290_historical_parity.json" --authorization "USER_APPROVED_SAFE_PORTFOLIO_LIVE_SIGNAL_2026_06_23"
set "RC=%ERRORLEVEL%"
