...
REM GOLD V3 58 bounded checkpoint replay dry run audit-only runner.
REM Applies Stage57 bounded replay contract only. Does not implement live trading.
...
set "CANDLE_DIR="
if exist "%REPO_ROOT%\..\..\FX_OUTPUTS\gold_v3\57_bounded_replay_window_freeze_decision_audit_only" set "CANDLE_DIR=%REPO_ROOT%\..\.."
if "%CANDLE_DIR%"=="" (
    if exist "%REPO_ROOT%\..\FX_OUTPUTS\gold_v3\57_bounded_replay_window_freeze_decision_audit_only" set "CANDLE_DIR=%REPO_ROOT%\.."
)
if "%CANDLE_DIR%"=="" (
    if exist "%REPO_ROOT%\Files\FX_OUTPUTS\gold_v3\57_bounded_replay_window_freeze_decision_audit_only" set "CANDLE_DIR=%REPO_ROOT%\Files"
)
if "%CANDLE_DIR%"=="" (
    if exist "%REPO_ROOT%\FX_OUTPUTS\gold_v3\57_bounded_replay_window_freeze_decision_audit_only" set "CANDLE_DIR=%REPO_ROOT%"
)
if "%CANDLE_DIR%"=="" (
    echo [ERROR] Could not locate Stage57 output directory.
    pause
    exit /b 1
)
...
set "OUTPUT_DIR=%CANDLE_DIR%\FX_OUTPUTS\gold_v3\58_bounded_checkpoint_replay_dry_run_audit_only"
...
if not exist "%STAGE57_DIR%\gold_v3_57_bounded_replay_window_contract.csv" (
    echo [ERROR] Stage57 bounded replay contract not found.
    pause
    exit /b 1
)
...
echo [GOLD V3 58 BOUNDED CHECKPOINT REPLAY DRY RUN AUDIT ONLY]
echo REPO_ROOT=%REPO_ROOT%
echo CANDLE_DIR=%CANDLE_DIR%
echo OUTPUT_DIR=%OUTPUT_DIR%
echo.
echo Applies Stage57 bounded replay contract. No live enablement.
echo.

python "%REPO_ROOT%\scripts\gold_v3_runtime\gold_v3_58_bounded_checkpoint_replay_dry_run_audit.py" ^
...
if not "%ERR%"=="0" (
    echo.
    echo [FAILED] GOLD V3 58 bounded checkpoint replay dry run failed with errorlevel %ERR%.
    pause
    exit /b %ERR%
)

echo.
echo [DONE] GOLD V3 58 outputs written to:
echo %OUTPUT_DIR%
echo.
echo Paste this file if upload limit is reached:
echo %OUTPUT_DIR%\gold_v3_58_PASTE_ME_BOUNDED_REPLAY_DRY_RUN_SUMMARY.txt
echo.
pause
exit /b 0
