...
REM GOLD V3 49 closed-asof state schema and shadow ledger audit-only runner.
REM Freezes schemas/templates only. Does not implement live trading.
...
set "OUTPUT_DIR=%CANDLE_DIR%\FX_OUTPUTS\gold_v3\49_closed_asof_state_schema_and_shadow_ledger_audit_only"
...
if not exist "%STAGE48_DIR%\gold_v3_48_live_readiness_gap_summary.json" (
    echo [ERROR] Stage48 output not found.
    pause
    exit /b 1
)
...
echo [GOLD V3 49 STATE SCHEMA AUDIT ONLY]
echo REPO_ROOT=%REPO_ROOT%
echo CANDLE_DIR=%CANDLE_DIR%
echo OUTPUT_DIR=%OUTPUT_DIR%
echo.
echo This freezes state schemas/templates only. No live enablement.
echo.

python "%REPO_ROOT%\scripts\gold_v3_runtime\gold_v3_49_closed_asof_state_schema_and_shadow_ledger_audit.py" ^
...
if not "%ERR%"=="0" (
    echo.
    echo [FAILED] GOLD V3 49 state schema audit failed with errorlevel %ERR%.
    pause
    exit /b %ERR%
)

echo.
echo [DONE] GOLD V3 49 outputs written to:
echo %OUTPUT_DIR%
echo.
echo Paste this file if upload limit is reached:
echo %OUTPUT_DIR%\gold_v3_49_PASTE_ME_STATE_SCHEMA_SUMMARY.txt
echo.
pause
exit /b 0
