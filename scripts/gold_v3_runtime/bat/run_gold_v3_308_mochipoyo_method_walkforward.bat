...
set "OUTPUT_JSON=%TRAIN_DIR%\stage308_mochipoyo_method_walkforward.json"
set "TRADES_CSV=%TRAIN_DIR%\stage308_mochipoyo_method_walkforward_trades.csv"
echo [INFO] Running Stage308 Mochipoyo-method walk-forward research...
%PYTHON_CMD% "%RUNTIME%\gold_v3_308_mochipoyo_method_walkforward.py" --candle-dir "%TRAIN_DIR%" --output "%OUTPUT_JSON%" --trades-csv "%TRADES_CSV%" --point-size 0.01 --top 200
set "RC=%ERRORLEVEL%"
echo.
echo Result JSON:
echo %OUTPUT_JSON%
