...
set "STAGE311_JSON=%TRAIN_DIR%\stage311_mochipoyo_and_independent_candidate_research.json"
set "STAGE311_TRADES=%TRAIN_DIR%\stage311_candidate_research_all_trades.csv"
set "OUTPUT_JSON=%TRAIN_DIR%\stage312_near_miss_candidate_refinement.json"
set "SELECTED_CSV=%TRAIN_DIR%\stage312_selected_refined_candidate_trades.csv"
echo Running Stage312 near-miss candidate refinement...
%PYTHON_CMD% "%RUNTIME%\gold_v3_312_near_miss_candidate_refinement.py" --stage311-json "%STAGE311_JSON%" --stage311-trades "%STAGE311_TRADES%" --output "%OUTPUT_JSON%" --selected-csv "%SELECTED_CSV%"
set "RC=%ERRORLEVEL%"
echo.
echo %OUTPUT_JSON%
echo %SELECTED_CSV%
