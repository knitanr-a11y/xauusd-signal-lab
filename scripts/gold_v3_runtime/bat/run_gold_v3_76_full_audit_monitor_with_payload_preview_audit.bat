...
set "OUTPUT_DIR=%CANDLE_DIR%\FX_OUTPUTS\gold_v3\76_full_audit_monitor_with_payload_preview_audit_only"
for %%I in ("%OUTPUT_DIR%") do set "OUTPUT_DIR=%%~fI"
if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"

echo [GOLD V3 76 FULL AUDIT MONITOR WITH PAYLOAD PREVIEW AUDIT ONLY]
...
set "ERR=%ERRORLEVEL%"
echo.
echo [STOPPED] GOLD V3 76 full audit monitor exited with errorlevel %ERR%.
echo Paste this file if upload limit is reached:
echo %OUTPUT_DIR%\gold_v3_76_PASTE_ME_FULL_AUDIT_MONITOR_WITH_PAYLOAD_PREVIEW_SUMMARY.txt
echo.
pause
exit /b %ERR%
