...
def make_conditions(data:pd.DataFrame):
    ...
    for c in data.columns:
        ...
        if pd.api.types.is_numeric_dtype(data[c]) and data[c].notna().sum()>1000 and data[c].nunique(dropna=True)>20:
            if any(k in c for k in ['rsi','range_atr','body_atr','ret','dist','atr','body','wick','range']):
                num_cols.append(c)
    conds=[]
    for c in bool_cols:
    ...
    for c in num_cols:
        ...
        for _,v in qv.items():
            if np.isfinite(v):
                conds.append((f'{c}>={v:.6g}',arr>=v)); conds.append((f'{c}<={v:.6g}',arr<=v))
    return conds
...
def main()->int:
    ...
    top=pd.DataFrame(); allout=pd.DataFrame()
    if not blockers:
        ...
        for direction in ['LONG','SHORT']:
            for tp,sl,h in [(10,5,48),(15,7.5,64),(20,10,96),(25,10,96),(30,15,128),(40,20,192)]:
        ...
        for pi,(direction,tp,sl,h) in enumerate(profiles,1):
            ...
            pairs=[]; top_rules=[r['rule'] for r in singles[:80]]
        ...
        if not allout.empty:
            ...
            # add month stats for top rules
            rule_to_mask={n:m for n,m in zip(names,masks)}
            rows=[]
            for _,r in top.iterrows():
                pnl=compute_outcome(data,frames['m5'],r.direction,float(r.tp),float(r.sl),int(r.horizon_m5))
                ...
                rr=r.to_dict(); rr['full_months']=months_n; rr['full_neg_months']=neg_m; rr['beats_old_pf_2_237']=bool(r.full_pf>BENCHMARK_PF and r.test_pf>BENCHMARK_PF and r.train_pf>BENCHMARK_PF)
                rows.append(rr)
            top=pd.DataFrame(rows)
            save(allout,out/'gold_v3_177_all_passed_rules.csv'); save(top,out/'gold_v3_177_top100_rules.csv')
    ready=not blockers
    ...
    summary={'step':STEP,'status':'READY' if ready else 'BLOCKED','ready':ready,'decision':'OHLC_ONLY_REBUILD_SEARCH_READY' if ready else 'OHLC_ONLY_REBUILD_SEARCH_BLOCKED','created_at_utc':datetime.now(timezone.utc).isoformat(timespec='seconds').replace('+00:00','Z'),'output_dir':str(out),'audit_only':True,'review_only':True,'old_pf_benchmark':BENCHMARK_PF,'best_full_pf':best_pf,'best_rule':best_rule,'top_rows':int(len(top)) if ready else 0,'source_csv_mutated':False,'contract_mutated':False,'open_asof_allowed':False,'candidate_pool_removed':False,'f002_exclusion_bypassed':False,'final_live_enabled':False,'blocker_count':len(blockers),'elapsed_seconds':round(time.time()-t0,2)}
