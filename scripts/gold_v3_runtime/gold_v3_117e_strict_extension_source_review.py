...
def main():
    ...
    rank=pd.DataFrame(); review_ext=pd.DataFrame(); best={}; decision=''
    if not blockers:
    ...
    if not blockers:
        ...
        if best and ext_path.exists():
            ...
            if '_source_rel_path' in ext.columns and src:
                review_ext=ext[ext['_source_rel_path'].astype(str)==src].copy()
            else:
                review_ext=ext.iloc[0:0].copy()
    ...
    summary=dict(step=STEP,status=status,ready=status==READY,decision=decision or ('BLOCKED_INPUT_INCOMPLETE' if blockers else ''),created_at_utc=datetime.now(timezone.utc).isoformat(timespec='seconds').replace('+00:00','Z'),output_dir=str(out),best_rel_path=str(best.get('rel_path','')) if best else '',best_recall=float(best.get('selected_may_entry_recall',0)) if best else 0.0,best_precision=float(best.get('selected_may_entry_precision',0)) if best else 0.0,best_extra_may_entries=int(float(best.get('projected_extra_may_entries',0))) if best else 0,best_projected_june_rows=int(float(best.get('projected_june_rows',0))) if best else 0,review_extension_rows=review_rows,strict_auto_gate_count=int(rank['strict_auto_gate'].sum()) if not rank.empty and 'strict_auto_gate' in rank.columns else 0,review_gate_count=int(rank['review_gate'].sum()) if not rank.empty and 'review_gate' in rank.columns else 0,source_csv_mutated=False,contract_mutated=False,open_asof_allowed=False,approximate_reconstruction=False,blocker_count=len(blockers),elapsed_seconds=round(time.time()-t0,2))
