...
def numeric_metrics(orig: pd.Series, cand: pd.Series) -> dict:
...
def bool_metrics(orig: pd.Series, cand: pd.Series) -> dict:
    a=bool_series(orig); b=bool_series(cand)
    ok=orig.notna() & cand.notna()
...
def asof_series(entry_dt: pd.Series, src: pd.DataFrame, value: pd.Series, name: str) -> pd.Series:
    left = pd.DataFrame({'entry_dt': entry_dt}).reset_index(names='_row_id')
    ...
    m = m.sort_values('_row_id')
    return m[name].reset_index(drop=True)

def shifted_asof(entry_dt: pd.Series, src: pd.DataFrame, base: pd.Series, name: str, shift: int) -> pd.Series:
    return asof_series(entry_dt, src, base.shift(shift), f'{name}__shift{shift}')

def add_numeric_rank(rows: list[dict], feature: str, candidate_col: str, orig: pd.Series, cand: pd.Series) -> None:
    rows.append({'feature':feature,'candidate_col':candidate_col,**parse_variant(candidate_col),**numeric_metrics(orig,cand)})

def add_bool_rank(rows: list[dict], candidate_col: str, orig: pd.Series, cand: pd.Series) -> None:
    rows.append({'feature':'h1_up','candidate_col':candidate_col,**parse_variant(candidate_col),**bool_metrics(orig,cand)})
...
def main() -> int:
    ...
    missing=[c for c in ['entry_dt','m15_rsi14','h1_range_atr','d1_dist_atr','h1_up'] if c not in ledger.columns]
    if missing: blockers.append({'id':'missing_original_feature_columns','missing':missing})
    num_rows=[]; bool_rows=[]; best_num=pd.DataFrame(); best_bool=pd.DataFrame()
    if not blockers:
        x=ledger[['entry_dt','m15_rsi14','h1_range_atr','d1_dist_atr','h1_up']].copy()
        x['entry_dt']=pd.to_datetime(x.entry_dt, errors='coerce')
        x=x[x.entry_dt.notna()].sort_values('entry_dt').reset_index(drop=True)
        entry=x['entry_dt']
        # M15 RSI candidates: one candidate at a time; no wide merge.
        rsi_candidates={
            'm15_rsi_ewm14_adjustFalse': rsi_ewm(m15.close,14,False),
            'm15_rsi_ewm14_adjustTrue': rsi_ewm(m15.close,14,True),
            'm15_rsi_sma14': rsi_sma(m15.close,14),
        }
        for base_name, ser in rsi_candidates.items():
            for sh in SHIFTS:
                cname=f'{base_name}__shift{sh}'
                add_numeric_rank(num_rows,'m15_rsi14',cname,x['m15_rsi14'],shifted_asof(entry,m15,ser,base_name,sh))
        # H1 range/ATR candidates.
        h1_range=(h1.high-h1.low)
        for p in PERIODS:
            atrs={f'h1_range_atr_sma{p}': h1_range/atr_sma(h1,p), f'h1_range_atr_ewmA0_{p}': h1_range/atr_ewm(h1,p,False), f'h1_range_atr_ewmA1_{p}': h1_range/atr_ewm(h1,p,True)}
            for base_name, ser in atrs.items():
                for sh in SHIFTS:
                    cname=f'{base_name}__shift{sh}'
                    add_numeric_rank(num_rows,'h1_range_atr',cname,x['h1_range_atr'],shifted_asof(entry,h1,ser,base_name,sh))
        # H1 trend candidates.
        ...
        h1up_candidates={
            'h1_up_close_gt_prev': h1.close>h1.close.shift(1),
            'h1_up_close_gt_open': h1.close>h1.open,
            'h1_up_ema20_gt_ema50': ema20>ema50,
            'h1_up_close_gt_sma20': h1.close>sma20,
            'h1_up_close_delta3_gt0': (h1.close-h1.close.shift(3))>0,
        }
        for base_name, ser in h1up_candidates.items():
            for sh in SHIFTS:
                cname=f'{base_name}__shift{sh}'
                add_bool_rank(bool_rows,cname,x['h1_up'],shifted_asof(entry,h1,ser,base_name,sh))
        # D1 distance candidates. Get M15 close once by asof, then compare against D1 refs/ATRs one at a time.
        m15_close=asof_series(entry,m15,m15.close,'m15_close_for_d1')
        ...
        for refn, ref in refs.items():
            for atrn, atrser in atrs.items():
                base_name=f'd1_ref_{refn}_atr_{atrn}__dist'
                for sh in SHIFTS:
                    ref_asof=shifted_asof(entry,d1,ref,base_name+'_ref',sh)
                    atr_asof=shifted_asof(entry,d1,atrser,base_name+'_atr',sh)
                    cand=(pd.to_numeric(m15_close,errors='coerce')-pd.to_numeric(ref_asof,errors='coerce'))/pd.to_numeric(atr_asof,errors='coerce')
                    cname=f'{base_name}__shift{sh}'
                    add_numeric_rank(num_rows,'d1_dist_atr',cname,x['d1_dist_atr'],cand)
        ...
        if not boo.empty:
            boo=boo.sort_values(['feature','mismatch_rate','candidate_col'], ascending=[True,True,True])
