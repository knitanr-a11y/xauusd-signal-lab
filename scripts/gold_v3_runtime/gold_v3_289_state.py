...
def evaluate_shadow_eligibility(candidates: pd.DataFrame, ledger: pd.DataFrame, base: pd.DataFrame, base_state_ready: bool=True) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for r in candidates.sort_values(['entry_dt', 'priority']).itertuples(index=False):
        ...
        rec = r._asdict()
        rec.update({'shadow_eligible': accepted, 'reject_reasons': ';'.join(reasons), 'state_dd': st['dd'], 'state_equity': st['equity'], 'base_resolved_count': st['base_resolved_count']})
        ...
        if accepted:
            ...
            trade = rec.copy()
            trade.update({'tp_price': tp, 'sl_price': sl, 'status': 'OPEN', 'exit_dt': pd.NaT, 'exit_price': np.nan, 'exit_reason': '', 'gross_pnl': np.nan, 'pnl': np.nan})
