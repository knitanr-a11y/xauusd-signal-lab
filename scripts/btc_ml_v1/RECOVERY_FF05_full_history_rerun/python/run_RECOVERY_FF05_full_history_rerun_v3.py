...
spec = importlib.util.spec_from_file_location("ff05_recovery_rerun_v2", V2_PATH)
...
def _copy_prerequisite_outputs(isolated_local_base: Path) -> list[str]:
    """Mirror only small frozen prerequisite outputs needed by the FF05 runner."""
    copied: list[str] = []
    ...
    for source in REAL_LOCAL_BASE.iterdir():
        if not source.is_dir() or not source.name.startswith(allowed_prefixes):
            continue
        target = isolated_local_base / source.name
        if target.exists():
    ...
    if not any(name.startswith("04_") for name in copied):
        raise RuntimeError(
            "FF04 prerequisite output was not mirrored into isolated LOCALAPPDATA"
        )
    return sorted(copied)


def create_isolated_terminal_v3(
    output_root: Path,
    manifest,
):
    """
    Build a fully isolated Windows profile and LOCALAPPDATA tree.

    The previous attempt still exposed the real user through LOCALAPPDATA-derived
    discovery. V3 isolates every home/appdata route, mirrors only the frozen
    prerequisite result folders, and places merged CSVs under every supported
    MetaQuotes root inside the isolated profile.
    """
    profile_root = output_root / "_isolated_profile_v3"
    ...
    mirrored_prerequisites = _copy_prerequisite_outputs(isolated_output_base)
    ...
    for row in manifest.to_dict(orient="records"):
        ...
        primary_target = roaming / "MetaQuotes" / "Terminal" / "FF05_MERGED_FULL_HISTORY" / "MQL5" / "Files" / source.name
        for files_dir in terminal_roots:
...
module.create_isolated_terminal = create_isolated_terminal_v3
