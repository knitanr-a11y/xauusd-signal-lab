...
DEFAULT_M15_CSV = PROJECT_ROOT / "data" / "raw" / "btcusdsharp_m15_avg_spread.csv"
DEFAULT_H1_CSV = PROJECT_ROOT / "data" / "raw" / "btcusdsharp_h1.csv"
DEFAULT_OUT_SUMMARY = PROJECT_ROOT / "data" / "results" / "btcusdsharp_value_width_edge_summary.csv"
DEFAULT_OUT_TRADES = PROJECT_ROOT / "data" / "results" / "btcusdsharp_value_width_edge_trades.csv"
...
def join_h1(m15: pd.DataFrame, h1: pd.DataFrame) -> pd.DataFrame:
    h1_cols = ["time", "ema_align", "macd_hist", "macd_delta3", "rci26", "rci26_delta", "rci52", "rci52_delta"]
...
def build_value_width_rules(df: pd.DataFrame) -> list[tuple[str, str, pd.Series, pd.Series]]:
    ...
    standard_rci_buy = (df["rci9"] <= 0) & (df["rci9_delta"] > 0) & (df["rci26"] >= -60)
    standard_rci_sell = (df["rci9"] >= 0) & (df["rci9_delta"] < 0) & (df["rci26"] <= 60)

    not_overextended = df["close_change_3_atr"].abs() <= 1.20
    gap_buy = df["close_ema20_gap_atr"].between(-0.20, 0.50)
    gap_sell = df["close_ema20_gap_atr"].between(-0.50, 0.20)

    trend_pull_runner_buy = h1_bull & h1_macd_buy & ema20_pull_buy & m15_macd_buy & standard_rci_buy & not_overextended & gap_buy
    trend_pull_runner_sell = h1_bear & h1_macd_sell & ema20_pull_sell & m15_macd_sell & standard_rci_sell & not_overextended & gap_sell

    # Mean-reversion candidates are intentionally included for diagnostics.
    # In current BTC data they tend to be weak, but this makes that visible in output.
    mr_buy = (
        (df["close_ema20_gap_atr"] < -1.50)
        & (df["rci9"].shift(1) < -80)
        & (df["rci9"] > -80)
        & (df["rci9_delta"] > 0)
    ...
    return [
        (
            "trend_pull_runner",
            "H1方向 + EMA20押し戻し + MACD再加速 + RCI反転。値幅狙いの本命候補。",
            trend_pull_runner_buy,
            trend_pull_runner_sell,
        ),
        (
            "mean_reversion_extreme_rci",
            "逆張り診断用。EMA20から1.5ATR以上乖離 + RCI9極端から反転 + ヒゲ確認。",
            mr_buy,
            mr_sell,
        ),
        (
            "breakout_runner",
            "ブレイク伸び狙い診断用。H1方向 + M15 20本ブレイク + RCI/MACD方向。",
            breakout_runner_buy,
            breakout_runner_sell,
        ),
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Search BTCUSD# value-width edges after removing WIDE from core notify tiers.")
    ...
    parser.add_argument("--rr-values", default="2.0,2.5,3.0")
    parser.add_argument("--risk-atr-values", default="1.0,1.2,1.5")
    ...
    for base_rule_name, description, buy_mask, sell_mask in build_value_width_rules(df):
        for rr in rr_values:
    ...
    print_table("BTCUSD# VALUE-WIDTH EDGE SUMMARY", summary[display_cols])
    return 0
