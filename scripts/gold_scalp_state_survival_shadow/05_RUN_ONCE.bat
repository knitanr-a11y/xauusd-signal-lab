@echo off
setlocal
title GOLD P75 State Survival Shadow - Run Once
...
echo ============================================================
echo GOLD P75 STATE SURVIVAL SHADOW - RUN ONCE
echo Recovery/audit only / MT5 orders OFF
echo ============================================================
if not exist "%PY%" (
...
pushd "%ROOT%"
"%PY%" -m scripts.gold_scalp_state_survival_shadow.shadow_runtime run-once --config "%CFG%"
set "RC=%ERRORLEVEL%"
popd
