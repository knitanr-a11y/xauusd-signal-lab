...
def run(artifact_dir: Path, manifest_path: Path, output_dir: Path) -> int:
    ...
    hashes_path = manifest_path.with_name("artifact_hashes.csv")
    if not hashes_path.is_file():
        hashes_path = manifest_path.parents[4] / manifest["artifact_hashes"]
    hash_rows = pd.read_csv(hashes_path)
    manifest["files"] = {
        row.filename: {"local_sha256": row.sha256, "bytes": int(row.bytes)}
        for row in hash_rows.itertuples(index=False)
    }
    required = sorted(manifest["files"])
    missing = [name for name in required if not (artifact_dir / name).is_file()]
    if missing:
        raise FileNotFoundError(
            "Missing frozen research-challenger artifacts. Install the exact local audit package first: "
            + ", ".join(missing)
        )
    ...
    report = {
        ...
        "notes": [
            ...
            "Normalized output fills only known WATCH-022-C A_CORE identity omissions.",
            "This is artifact-exact parity, not raw-candle regeneration for approximate P16/P18/P19.",
            "No unrelated ML implementation is substituted.",
        ],
    }
