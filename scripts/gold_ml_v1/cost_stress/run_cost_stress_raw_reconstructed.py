...
from cost_stress_engine import M1Engine, build_risk_lookup, replay
...
def run(args: argparse.Namespace) -> int:
    ...
    if len(trades) != expected:
        raise RuntimeError(f"Trade/scenario rows {len(trades)} != {expected}")

    candidate = candidate_summary(trades, config)
    year = year_summary(trades, config)
    lineage = lineage_summary(candidate)
    gate = overall_gate(candidate, len(scenarios))
    if set(gate.candidate_id.astype(str)) != set(candidates):
        raise RuntimeError("Overall gate candidate set mismatch")

    write_csvs(output, trades, candidate, year, lineage, gate)
    ...
    provenance = {
        ...
        "baseline_parity_checks": checks,
        "scenario_count": len(scenarios),
        "trade_scenario_rows": len(trades),
        "audit_only": True,
    }
    ...
    summary = {
        ...
        "scenario_grid": [item.__dict__ for item in scenarios],
        "baseline_parity_checks": checks,
        "candidate_overall_gate": records(gate),
        "candidate_overall_gate_counts": {"PASS": passes, "FAIL": fails},
        "raw_candidate_scenario_results": records(candidate[candidate.population == RAW]),
        "bridge_candidate_scenario_results": records(candidate[candidate.population == BRIDGE]),
    ...
    print(f"Baseline parity checks: {checks}")
    print(f"Candidate stress gate: PASS={passes} FAIL={fails}")
