@echo off
setlocal
set "STATE_ROOT=%LOCALAPPDATA%\xauusd_signal_lab\gold_late_transition_v1_shadow"
set "VENV=%STATE_ROOT%\venv"
if not exist "%STATE_ROOT%" mkdir "%STATE_ROOT%"
where py >nul 2>&1 || (
  echo [BLOCKED] Python launcher 'py' was not found.
  pause
  exit /b 2
)
if not exist "%VENV%\Scripts\python.exe" (
  py -3.12 -m venv "%VENV%" || py -3 -m venv "%VENV%" || (
    echo [BLOCKED] Could not create the Python virtual environment.
    pause
    exit /b 2
  )
)
"%VENV%\Scripts\python.exe" -m pip install --upgrade pip || goto :fail
"%VENV%\Scripts\python.exe" -m pip install -r "%~dp0requirements.txt" || goto :fail
echo [OK] GOLD Late Transition V1 environment installed at:
echo %VENV%
pause
exit /b 0
:fail
echo [BLOCKED] Installation failed. Review the error above.
pause
exit /b 2
