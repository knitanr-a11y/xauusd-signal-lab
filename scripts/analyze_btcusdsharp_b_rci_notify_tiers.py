...
DEFAULT_OUT_SUMMARY = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_notify_tier_summary.csv"
DEFAULT_OUT_TRADES = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_notify_tier_trades.csv"
DEFAULT_OUT_RULES = PROJECT_ROOT / "data" / "results" / "btcusdsharp_b_rci_notify_tier_rules.md"
...
def base_masks(df: pd.DataFrame, *, ema_touch_atr: float, buy_gap_min: float, buy_gap_max: float, sell_gap_min: float, sell_gap_max: float, rci9_buy_max: float, rci9_sell_min: float, rci26_buy_min: float, rci26_sell_max: float, max_abs_close_change3: float | None, require_rci26_delta: bool = False, require_h1_rci26_side: bool = False, min_body_ratio: float | None = None) -> tuple[pd.Series, pd.Series]:
    ...
    buy = h1_bull & h1_macd_buy & near_ema20_buy & m15_macd_buy & gap_buy & rci_buy & not_extended & body_ok
    sell = h1_bear & h1_macd_sell & near_ema20_sell & m15_macd_sell & gap_sell & rci_sell & not_extended & body_ok
    return buy, sell


def tier_masks(df: pd.DataFrame) -> dict[str, tuple[pd.Series, pd.Series, str]]:
    """Fixed BTC B/RCI notification tiers.

    These were selected from the high-win-rate search:
    - HIGH: strictest high-win-rate style.
    - STANDARD: balance style, roughly 80 trades / 66% win-rate in the search.
    - WIDE: total-R style, broader than STANDARD.

    Tiers are hierarchical. A signal matching HIGH is not also counted as STANDARD/WIDE.
    """
    high_buy, high_sell = base_masks(
        ...
        require_rci26_delta=False,
        require_h1_rci26_side=False,
        min_body_ratio=None,
    )
...
def backtest_tier_signals(df: pd.DataFrame, *, rr: float, risk_atr: float, max_bars: int, cooldown_bars: int, start_bar: int) -> pd.DataFrame:
...
def write_rules(path: Path) -> None:
    content = """# BTCUSD# B/RCI Notify Tier Rules

These rules are intended for notification ranking, not automatic execution.

All tiers share the same base idea:

```text
H1 EMA direction agrees with trade side
H1 MACD histogram delta3 agrees with trade side
M15 touches/approaches EMA20 and closes back on the trend side
M15 MACD histogram delta and delta3 re-accelerate in trade direction
M15 RCI9 turns from a pullback/rebound area
M15 RCI26 is not strongly adverse
M15 recent 3-candle move is not too extended
```
...
```text
ema_touch_atr = 0.30
BUY gap from EMA20: -0.20 to +0.50 ATR
SELL gap from EMA20: -0.50 to +0.20 ATR
BUY RCI9 <= -20 and RCI9 rising
SELL RCI9 >= +20 and RCI9 falling
BUY RCI26 >= -60
SELL RCI26 <= +60
...
BUY gap from EMA20: -0.20 to +0.50 ATR
SELL gap from EMA20: -0.50 to +0.20 ATR
BUY RCI9 <= 0 and RCI9 rising
SELL RCI9 >= 0 and RCI9 falling
BUY RCI26 >= -60
SELL RCI26 <= +60
abs(M15 close_change_3_atr) <= 1.20
```
...
```text
ema_touch_atr = 0.30
BUY gap from EMA20: -0.20 to +0.80 ATR
SELL gap from EMA20: -0.80 to +0.20 ATR
BUY RCI9 <= 0 and RCI9 rising
SELL RCI9 >= 0 and RCI9 falling
BUY RCI26 >= -80
SELL RCI26 <= +80
...
```text
WIDE includes broader conditions than STANDARD.
STANDARD includes broader conditions than HIGH.
The script assigns only the highest matching tier to avoid duplicate notifications.
```
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze BTCUSD# B/RCI hierarchical notification tiers.")
    ...
    print_table("BTCUSD# B/RCI NOTIFY TIER SUMMARY", summary[display_cols] if not summary.empty else summary)
