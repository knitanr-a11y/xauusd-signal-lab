...
This script does NOT call OpenAI, does NOT send Discord, and does NOT send MT5 orders.
...
- max total 640 rows
- deterministic balanced selection by split/month/loss bucket
"""
...
def balanced_take(df: pd.DataFrame, limit: int) -> pd.DataFrame:
    ...
    for idx, row in work.iterrows():
        key = (str(row.get("split", "unknown")), str(row.get("entry_month", "unknown")), str(row.get("review_bucket", "unknown")))
    ...
    return out.sort_values("entry_time_dt", kind="mergesort").drop(columns=["entry_time_dt"], errors="ignore").reset_index(drop=True)
...
def build_sample(trades: pd.DataFrame, candidate_ids: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    work = trades.copy()
    work["profit_r_num"] = pd.to_numeric(work.get("profit_r"), errors="coerce")
    work["outcome_upper"] = work.get("outcome", "").astype(str).str.upper()
    work["entry_time_dt"] = pd.to_datetime(work.get("entry_time"), errors="coerce")
    work["review_bucket"] = work.apply(classify_review_bucket, axis=1)
    ...
    for candidate_id in candidate_ids:
        ...
        non_loss = cand[~cand.index.isin(loss.index)].copy()
        ...
        sample = sample.sort_values("entry_time_dt", kind="mergesort").reset_index(drop=True)
        if len(sample) > MAX_PER_STRATEGY:
            sample = sample.head(MAX_PER_STRATEGY).copy()
        sample["ai_review_sample_selected"] = True
        sample["ai_review_sample_reason"] = sample["review_bucket"]
        rows.append(sample)

        rejected = cand[~cand.index.isin(sample.index)].copy()
    ...
    for df in [out, rejected_out]:
        if not df.empty:
            df.drop(columns=["profit_r_num", "outcome_upper", "entry_time_dt"], errors="ignore", inplace=True)
...
def summarize_strategy(full: pd.DataFrame, sample: pd.DataFrame, candidate_ids: list[str]) -> pd.DataFrame:
    ...
    full_work = full.copy()
    full_work["profit_r_num"] = pd.to_numeric(full_work.get("profit_r"), errors="coerce")
    full_work["outcome_upper"] = full_work.get("outcome", "").astype(str).str.upper()
    sample_work = sample.copy()
    sample_work["profit_r_num"] = pd.to_numeric(sample_work.get("profit_r"), errors="coerce") if not sample_work.empty else pd.Series(dtype=float)
    sample_work["outcome_upper"] = sample_work.get("outcome", pd.Series(dtype=str)).astype(str).str.upper() if not sample_work.empty else pd.Series(dtype=str)
    for candidate_id in candidate_ids:
        ...
        f_loss = f[(f["outcome_upper"].eq("LOSS")) | (f["profit_r_num"] < 0)]
        s_loss = s[(s["outcome_upper"].eq("LOSS")) | (s["profit_r_num"] < 0)] if not s.empty else s
        months = pd.to_datetime(s.get("entry_time"), errors="coerce").dt.strftime("%Y-%m").nunique() if not s.empty else 0
        rows.append({
            ...
            "earliest_entry": "" if s.empty else str(pd.to_datetime(s["entry_time"], errors="coerce").min()),
            "latest_entry": "" if s.empty else str(pd.to_datetime(s["entry_time"], errors="coerce").max()),
...
def monthly_distribution(sample: pd.DataFrame) -> pd.DataFrame:
    ...
    work = sample.copy()
    work["entry_month"] = work["entry_month"] if "entry_month" in work.columns else pd.to_datetime(work["entry_time"], errors="coerce").dt.strftime("%Y-%m")
    work["split"] = work["split"] if "split" in work.columns else "unknown"
    work["review_bucket"] = work["review_bucket"] if "review_bucket" in work.columns else work.apply(classify_review_bucket, axis=1)
    return work.groupby(["candidate_id", "entry_month", "split", "review_bucket"], dropna=False).size().reset_index(name="rows").sort_values(["candidate_id", "entry_month", "split", "review_bucket"], kind="mergesort")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fail-on-missing-candidates", action="store_true", default=True)
    return p.parse_args()
...
def main() -> int:
    ...
    run_dir = args.out_root / datetime.now().strftime("%Y") / datetime.now().strftime("%m") / stamp
    ...
    audit = {
        ...
        "ok": bool(not missing and len(sample) <= MAX_TOTAL and summary["ok_sample_total_le_80"].all() and summary["ok_sample_losses_le_45"].all() and summary["ok_sample_non_losses_le_35"].all()),
    }
    ...
    show_cols = ["candidate_id", "full_trades", "full_losses", "full_non_losses", "sample_total", "sample_losses", "sample_non_losses", "sample_train_count", "sample_test_count", "sample_months"]
