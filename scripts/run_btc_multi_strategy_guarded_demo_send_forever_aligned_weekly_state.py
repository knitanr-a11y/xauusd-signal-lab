...
This is the BTC counterpart of the GOLD multi-strategy weekly-state runner, but
it is fully separated by BAT, logs, and persistent state.

Logs:
  data/runtime_logs/btc/YYYY/MM/week_XX/multi_strategy_btc/loop
...
LOOP_LOG_COLUMNS = [
    ...
    "guarded_sender_sent_rows", "position_policy", "max_symbol_positions", "max_symbol_lot",
    "persistent_order_ledger_csv", "child_order_ledger_csv", "ledger_synced_from_state",
    "ledger_synced_to_state", "total_seconds", "next_run_utc", "stdout_log", "stderr_log",
    "summary_json",
]
...
def main() -> int:
    ...
    try:
        while args.max_cycles == 0 or cycle_index < args.max_cycles:
            ...
            row = {
                "cycle_index": cycle_index, "cycle_start_utc": cycle_start, "cycle_end_utc": cycle_end,
                ...
                "persistent_order_ledger_csv": str(persistent_ledger), "child_order_ledger_csv": str(child_ledger),
                "ledger_synced_from_state": bool(synced_from), "ledger_synced_to_state": bool(synced_to),
                "total_seconds": as_float(child.get("timing", {}).get("total_seconds") if isinstance(child.get("timing"), dict) else elapsed, elapsed),
                "next_run_utc": next_run, "stdout_log": str(stdout_log), "stderr_log": str(stderr_log),
