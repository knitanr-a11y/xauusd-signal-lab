...
SCHEMA_VERSION = "btc_strict_5_official_guarded_demo_autotrade_v1"
...
def main() -> int:
    ...
    write_csv(__import__("pandas").DataFrame(payloads, columns=PAYLOAD_COLUMNS + ["filter_variant"]), payload_csv)
    ...
    if not payloads:
        ...
        (run_dir / "sender_stdout.log").write_text(proc.stdout or "", encoding="utf-8")
        (run_dir / "sender_stderr.log").write_text(proc.stderr or "", encoding="utf-8")
        (run_dir / "sender_command.txt").write_text(" ".join(cmd), encoding="utf-8")
    ...
    summary = {
        ...
        "safety": {"mt5_calls_direct_in_wrapper": False, "order_send_direct_in_wrapper": False, "discord_send": False, "ai_calls": False},
    }
