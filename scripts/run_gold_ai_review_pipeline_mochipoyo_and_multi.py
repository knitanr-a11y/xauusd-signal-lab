#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""Run the unified GOLD trade AI review pipeline for Mochipoyo + multi-strategy.

This is the same AI review architecture used for the Mochipoyo GOLD journal,
extended explicitly to include the GOLD multi-strategy guarded demo order ledger.

Pipeline:
1. Export read-only MT5 closed trade history.
2. Build deterministic trade_outcome_ledger.csv from one or more order ledgers.
3. Build feature snapshots with pre-entry M15=100 bars and post-entry M15=20 bars.
4. Build OpenAI-ready AI review payloads.
5. Run AI review from payloads using OPENAI_API_KEY from .env unless --dry-run.
6. Summarize AI hypothesis tags into trade_ai_tag_summary.csv.
...
- Multi and Mochipoyo order ledgers are read as inputs only.
...
Default outputs remain the shared trade_ai_review directory so the existing
Discord warning engine can read:
- data/runtime_logs/trade_ai_review/trade_outcome_ledger.csv
...
def parse_order_ledgers(values: list[str]) -> list[Path]:
    ...
    return [DEFAULT_MOCHIPOYO_ORDER_LEDGER, DEFAULT_MULTI_ORDER_LEDGER]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run unified GOLD AI review pipeline for Mochipoyo and multi-strategy.")
    p.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    p.add_argument("--mql5-files-dir", type=Path, default=DEFAULT_MQL5_FILES_DIR)
    p.add_argument("--order-ledger-csv", action="append", default=[], help="Order ledger CSV. Repeat to override defaults.")
    p.add_argument("--allow-missing-order-ledger", action="store_true")
    p.add_argument("--expected-login", type=int, default=75539039)
    p.add_argument("--broker-symbols", default="GOLD#", help="Symbols for MT5 history export, comma-separated.")
...
def main() -> int:
    ...
    paths = {
        ...
        "pipeline_summary_json": args.out_dir / "gold_ai_review_pipeline_summary.json",
    }
    ...
    if not order_ledgers:
        raise SystemExit("No existing order ledger CSVs found. Cannot build outcome ledger.")
    ...
    print("=" * 80, flush=True)
    print("Unified GOLD AI review pipeline - Mochipoyo + multi-strategy", flush=True)
    ...
    if not args.skip_ai_review:
        ...
        steps.append(run_cmd("run_trade_ai_review_from_payloads", review_cmd, allow_failure=False))
        if not steps[-1]["ok"]:
            raise SystemExit(1)
    else:
    ...
    if not path_exists(paths["review_jsonl"]):
        # Create an empty file so the summary script reports 0 reviews cleanly.
    ...
    pipeline_summary = {
        "schema_version": "gold_ai_review_pipeline_mochipoyo_and_multi_v1",
    ...
    print("=" * 80, flush=True)
    print("Unified GOLD AI review pipeline summary", flush=True)
    print(json.dumps({
        "cycle_ok": True,
        "outcome_rows": pipeline_summary["key_metrics"]["outcome_rows"],
        "outcome_matched_rows": pipeline_summary["key_metrics"]["outcome_matched_rows"],
