...
DEFAULT_OUT_SUMMARY = PROJECT_ROOT / "data" / "results" / "gold_btc_final_portfolio_summary.csv"
DEFAULT_OUT_TRADES = PROJECT_ROOT / "data" / "results" / "gold_btc_final_portfolio_trades.csv"
DEFAULT_OUT_MONTHLY = PROJECT_ROOT / "data" / "results" / "gold_btc_final_portfolio_monthly.csv"

DEFAULT_BTC_RULE = "trend_pull_runner_rr2.0_risk1.0"
DEFAULT_GOLD_OUTPUT_SET = "combined_high_quality_first"

FINAL_GOLD_KEEP_RANKS = {
    "GOLD_ABC",
    "GOLD_EXTRA_HIGH",
    "GOLD_EXTRA_STANDARD",
}

FINAL_GOLD_EXCLUDE_LABELS = {
    "GOLD_EXTRA_BB_WIDTH",
}
...
def classify_gold_rank(df: pd.DataFrame) -> pd.Series:
    strategy = df.get("strategy_label", pd.Series("", index=df.index)).astype(str)
    return pd.Series(
        np.select(
            [
                strategy.str.contains("ABC", case=False, na=False),
                strategy.isin(["GOLD_EXTRA_HIGH_RSI_STOCH", "GOLD_COUNTER_BUY_ONLY"]),
                strategy.eq("GOLD_EXTRA_BB_BALANCE"),
                strategy.eq("GOLD_EXTRA_BB_WIDTH"),
            ],
...
def prepare_gold_final(df: pd.DataFrame, *, gold_output_set: str, include_width: bool) -> pd.DataFrame:
    out = df.copy()
    if "output_set" in out.columns:
        out = out[out["output_set"].astype(str).eq(gold_output_set)].copy()
        if out.empty:
            available = sorted(df["output_set"].dropna().astype(str).unique().tolist())
            raise ValueError(f"No rows for gold output_set={gold_output_set}. Available: {available}")
    ...
    out["portfolio_rank"] = classify_gold_rank(out)

    keep_ranks = set(FINAL_GOLD_KEEP_RANKS)
    if include_width:
        keep_ranks.add("GOLD_EXTRA_WIDTH")
    out = out[out["portfolio_rank"].isin(keep_ranks)].copy()
    if not include_width:
        out = out[~out["strategy_label"].astype(str).isin(FINAL_GOLD_EXCLUDE_LABELS)].copy()
    return out.reset_index(drop=True)


def prepare_btc_final(df: pd.DataFrame, *, btc_rule_name: str) -> pd.DataFrame:
    out = df.copy()
    if "rule_name" not in out.columns:
        raise ValueError("BTC trades CSV must contain rule_name")
    out = out[out["rule_name"].astype(str).eq(btc_rule_name)].copy()
    if out.empty:
        available = sorted(df["rule_name"].dropna().astype(str).unique().tolist())[:30]
        raise ValueError(f"No rows for btc rule_name={btc_rule_name}. First available: {available}")
    out["symbol_group"] = "BTC"
    out["strategy_label"] = "BTC_RUNNER_RR2_RISK1"
    out["portfolio_signal_label"] = "BTC_RUNNER_RR2_RISK1"
    out["portfolio_rank"] = "BTC_RUNNER"
    return out.reset_index(drop=True)
...
def build_summary(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    rows.append(summarize(combined, group_name="all", group_value="FINAL_GOLD_PLUS_BTC"))
...
def build_monthly(combined: pd.DataFrame) -> pd.DataFrame:
    ...
    for month, group in out.groupby("entry_month", dropna=False):
        ...
        for symbol, sub in group.groupby("symbol_group", dropna=False):
            ...
            rows.append(row)
        for rank, sub in group.groupby("portfolio_rank", dropna=False):
            row = summarize(sub.copy(), group_name="month_rank", group_value=str(month))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze final GOLD and BTC portfolio without weak WIDE tiers.")
    ...
    parser.add_argument("--gold-output-set", default=DEFAULT_GOLD_OUTPUT_SET)
    parser.add_argument("--btc-rule-name", default=DEFAULT_BTC_RULE)
    parser.add_argument("--include-gold-width", action="store_true", help="Include GOLD_EXTRA_WIDTH for comparison. Default excludes it.")
    ...
    gold = prepare_gold_final(read_trades(gold_path), gold_output_set=args.gold_output_set, include_width=args.include_gold_width)
    btc = prepare_btc_final(read_trades(btc_path), btc_rule_name=args.btc_rule_name)
    ...
    print("Include GOLD_EXTRA_WIDTH:", args.include_gold_width)
    ...
    print_table("FINAL GOLD + BTC PORTFOLIO SUMMARY", summary[display_cols])
    ...
    print_table("FINAL GOLD + BTC MONTHLY SUMMARY", monthly[monthly_display_cols].tail(48) if not monthly.empty else monthly)
