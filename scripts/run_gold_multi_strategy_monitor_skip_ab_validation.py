#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""A/B validate that monitor skip does not change signal detection.

This validator runs the GOLD multi-strategy Mochipoyo-loop dry-run wrapper twice:
...
- strategy-level latest_m15_close_time
- strategy-level candidate_count / latest_candidate_entry_time when present
- generated order/close intent presence
- payload rows and valid payload count

Expected allowed difference:

- monitor_reason may change from NO_DRY_RUN_SIGNAL_CREATED_ROWS to
  MONITOR_SKIPPED_NO_DRY_RUN_SIGNAL_CREATED_ROWS when there are no open dry-run
  signals to monitor.
...
def compare_summaries(baseline: dict[str, Any], optimized: dict[str, Any]) -> dict[str, Any]:
    ...
    compare_field(checks, failures, name="payload_rows_out", baseline=baseline.get("key_metrics", {}).get("payload_rows_out"), optimized=optimized.get("key_metrics", {}).get("payload_rows_out"))
    compare_field(checks, failures, name="valid_order_payloads", baseline=baseline.get("key_metrics", {}).get("valid_order_payloads"), optimized=optimized.get("key_metrics", {}).get("valid_order_payloads"))
    ...
    compare_field(checks, failures, name="router_signals_found_count", baseline=b_router.get("signals_found_count"), optimized=o_router.get("signals_found_count"))
    compare_field(checks, failures, name="router_open_order_intent_count", baseline=b_router.get("open_order_intent_count"), optimized=o_router.get("open_order_intent_count"))
    compare_field(checks, failures, name="router_close_intent_count", baseline=b_router.get("close_intent_count"), optimized=o_router.get("close_intent_count"))
    ...
    safety_checks = {
        ...
        "optimized_skip_flag_true": as_bool(optimized.get("skip_monitor_when_no_open_signals"), False),
    }
    ...
    return {
        "checks": checks,
        "failures": failures,
        "comparison_ok": len(failures) == 0,
    }


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="A/B validate monitor skip signal invariance.")
...
def main() -> int:
    ...
    for case in [baseline, optimized]:
        append_csv_row(args.out_dir / "monitor_skip_ab_case_log.csv", {k: v for k, v in case.items() if k != "summary"}, CASE_LOG_COLUMNS)

    comparison = compare_summaries(baseline.get("summary", {}), optimized.get("summary", {}))
    returncodes_ok = baseline["returncode"] == 0 and optimized["returncode"] == 0
    validation_ok = bool(returncodes_ok and comparison["comparison_ok"])
    ...
    summary = {
        ...
        "reason": "MONITOR_SKIP_SIGNAL_INVARIANCE_PASS" if validation_ok else "MONITOR_SKIP_SIGNAL_INVARIANCE_FAILED",
        ...
        "timing": {
            ...
            "optimized_total_seconds": o_timing.get("total_seconds", ""),
            "baseline_router_seconds": b_timing.get("router_seconds", ""),
            "optimized_router_seconds": o_timing.get("router_seconds", ""),
        },
        "safety": {
        ...
        "paths": {
            ...
            "summary_json": str(args.out_dir / "latest_gold_multi_strategy_monitor_skip_ab_validation_result.json"),
        },
        "cases": [
            {k: v for k, v in baseline.items() if k != "summary"},
            {k: v for k, v in optimized.items() if k != "summary"},
        ],
    }
    write_json(args.out_dir / "latest_gold_multi_strategy_monitor_skip_ab_validation_result.json", summary)
    print("=" * 80, flush=True)
    print("GOLD multi-strategy monitor skip A/B validation summary", flush=True)
    print(json.dumps({
        "validation_ok": validation_ok,
