...
8. Read `config/gold_ml_v1/batch023_provenance_failure_20260625.json`, `config/gold_ml_v1/batch023_uploaded_raw_forensic_audit_20260625.json`, `config/gold_ml_v1/batch023_warmup_bridge_pass_20260625.json`, `config/gold_ml_v1/batch023_local_warmup_bridge_implementation_20260625.json`, and `docs/gold_ml_v1/NEXT_CHAT_HANDOFF_GOLD_ML_V1_BATCH023_WARMUP_BRIDGE_PASS_20260625.md` before Batch023 work.
...
19. The verified local runner is `scripts/gold_ml_v1/replay/run_batch023_warmup_bridge.bat`; it performs raw/ZIP SHA checks, creates `.venv_batch023_bridge`, extracts expected registries temporarily, backs up prior outputs, and returns exit code 0 only on 9/9 parity PASS.
20. Replay V1-V5 and the ZIP-bundled script are not original/exact replay tools and must not be described as such.
21. Historical raw input remains `gold_v3_2023_2026_*.csv`; live closed-bar observation remains `goldsharp_*.csv`. Never mix historical rows into new live decisions.
22. Same-lineage candidates are not independent edges and must not be summed as a portfolio.
23. The 2026 sample is diagnostic only and cannot be used for retuning.
24. Remain audit-only. No live activation, registration, automatic promotion, MT5 order, Discord signal, AI API, or live hook.
25. Next work is cost stress on `RAW_RECONSTRUCTED` rows with bridge rows shown separately, then fresh prospective goldsharp observation.
