...
set "LOG=%CD%\BTC_AI_V1_DAY_OPEN_SHADOW_LAST_LOG.txt"
...
> "%LOG%" echo BTC AI V1 Day Open Matched-Pair Shadow V1 process
...
if not defined BTC_AI_V1_DAY_OPEN_SHADOW_DISCORD_WEBHOOK_URL (
  >> "%LOG%" echo ERROR: BTC_AI_V1_DAY_OPEN_SHADOW_DISCORD_WEBHOOK_URL is not set.
  goto :fail_4
)
...
echo Processing newly closed data and retrying the durable Discord outbox...
%PYTHON_CMD% "%RUNTIME%" process --m1 "%M1%" --m15 "%M15%" --h4 "%H4%" --d1 "%D1%" >> "%LOG%" 2>&1
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :runtime_fail
...
echo.
echo ===== SUCCESS =====
echo Shadow V1 processed the latest closed CSV data.
echo Pending Discord outbox items were retried by the runtime.
echo No MT5 order API is used.
echo Full log: BTC_AI_V1_DAY_OPEN_SHADOW_LAST_LOG.txt
echo ===================
echo.
pause
exit /b 0
...
echo =====================
echo.
echo Shadow processing did not complete. Do not delete or edit runtime state by hand.
echo Paste BTC_AI_V1_DAY_OPEN_SHADOW_LAST_LOG.txt into the chat.
pause
exit /b %RC%
