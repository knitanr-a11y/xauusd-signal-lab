...
class ExplorationGuardrailTests(unittest.TestCase):
    ...
    def test_search_multiplicity_and_candidate_pool_are_protected(self) -> None:
        ...
        self.assertIn(
            "forbidden",
            pool["new_exploration_before_cost_stress_and_fresh_confirmation"],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.cost_stress["candidate_pool"]["research_only_ids"],
            pool["research_only_ids"],
        )
    ...
    def test_data_leakage_and_bridge_use_are_blocked(self) -> None:
        ...
        self.assertEqual(
            self.cost_stress["time_and_execution_contract"]["bridge_execution"][
    ...
    def test_one_click_cost_stress_and_diagnostics_are_fail_closed(self) -> None:
        ...
        self.assertEqual(
            self.cost_stress["scenario_grid"]["spread_multipliers"],
            [1.0, 1.5, 2.0],
        )
        self.assertEqual(
            self.cost_stress["scenario_grid"]["fixed_slippage_points_per_side"],
            [0, 5, 10, 20],
        )
        self.assertTrue(self.cost_stress["scenario_grid"]["grid_frozen_before_execution"])
        self.assertEqual(
            self.cost_stress["scenario_grid"]["post_result_grid_change"], "forbidden"
        )
