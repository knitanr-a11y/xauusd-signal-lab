# GOLD V2 25C48 representative filter set review spec implementation note

Date: 2026-06-08

Repo: `knitanr-a11y/xauusd-signal-lab`

## Implemented step

```text
25C48_COREB_G1_REPRESENTATIVE_FILTER_SET_REVIEW_SPEC_AUDIT_ONLY
```

Mode: audit-only specification/review.

## What was implemented

Added a repository-backed audit-only Python script and BAT launcher for 25C48.

```text
scripts/gold_v2_runtime/audit_gold_v2_25c48_coreb_g1_representative_filter_set_review_spec_audit_only.py
scripts/gold_v2_runtime/bat/25C48_COREB_G1_REPRESENTATIVE_FILTER_SET_REVIEW_SPEC_AUDIT_ONLY.bat
```

The BAT keeps the command window open with `pause` on both success and STOP/error paths.

## Input files

25C48 reads from 25C47:

```text
FX_OUTPUTS/gold_v2_25c47_coreb_g1_filter_coverage_next_plan_audit_only/
```

Required 25C47 files:

```text
02_25c47_filter_coverage_next_plan_summary.json
04_25c47_contract_audit.csv
05_25c47_representative_candidate_review.csv
06_25c47_next_option_matrix.csv
07_25c47_execution_boundary_matrix.csv
08_25c47_gates.csv
09_25c47_next_step_plan.csv
```

25C48 also reads the 25C46 selected plan:

```text
FX_OUTPUTS/gold_v2_25c46_coreb_g1_filter_coverage_review_audit_only/05_25c46_selected_coverage_plan.csv
```

## Output directory

```text
FX_OUTPUTS/gold_v2_25c48_coreb_g1_representative_filter_set_review_spec_audit_only/
```

Expected outputs:

```text
00_不要_25c48_file_request_list.csv
01_25c48_GOLD_V2_COREB_G1_REPRESENTATIVE_FILTER_SET_REVIEW_SPEC_AUDIT_ONLY_REPORT.md
02_25c48_representative_filter_set_review_spec_summary.json
03_25c48_input_audit.csv
04_25c48_contract_audit.csv
05_25c48_representative_filter_set.csv
06_25c48_review_spec_matrix.csv
07_25c48_blocked_execution_matrix.csv
08_25c48_gates.csv
09_25c48_next_step_plan.csv
10_25c48_handoff_notes.csv
```

## Source-of-truth facts

25C48 preserves the 25C47 representative candidate facts:

```text
representative_variant_code = A002
representative_retention_priority_cutoff = 1
representative_total_unique_damage_keys = 69
representative_covered_unique_keys = 69
representative_open_unique_keys = 0
representative_retained_filter_count = 2
representative_approval_status = NOT_APPROVED_REVIEW_ONLY
```

The representative filter set is read from 25C46 selected plan and must exactly match:

```text
same_count>=2&unique_origins>=2
unique_origins>=2
```

## Success conditions

25C48 succeeds only when:

```text
required 25C47 and 25C46 files exist
25C47 summary matches expected status and next step
25C47 audit matrices have no STOP rows
25C46 selected plan has exactly one selected representative
selected representative is A002 cutoff 1
selected representative has 69 covered keys and 0 open keys
selected representative has retained_filter_count = 2
selected representative remains NOT_APPROVED_REVIEW_ONLY
retained_filters exactly match the two expected filters
all execution and external action flags remain false
```

Successful status:

```text
COREB_G1_REPRESENTATIVE_FILTER_SET_REVIEW_SPEC_READY_AUDIT_ONLY
```

## Stop conditions

```text
25C48_STOP_MISSING_INPUT_AUDIT_ONLY
25C48_STOP_25C47_CONTRACT_UNSAFE_AUDIT_ONLY
25C48_STOP_REPRESENTATIVE_FILTER_SET_UNSAFE_AUDIT_ONLY
```

## Run order

From the repository root:

```bat
scripts\gold_v2_runtime\bat\25C48_COREB_G1_REPRESENTATIVE_FILTER_SET_REVIEW_SPEC_AUDIT_ONLY.bat
```

Or:

```bat
python scripts\gold_v2_runtime\audit_gold_v2_25c48_coreb_g1_representative_filter_set_review_spec_audit_only.py
```

## Next step

25C48 may recommend only this next audit-only specification step:

```text
25C49_COREB_G1_REPRESENTATIVE_FILTER_SET_DRY_RUN_SPEC_AUDIT_ONLY
```

25C49 must still be a specification step unless a later explicit approval changes the boundary.

## Explicit boundaries

25C48 does not approve A002/A004 or any variant.

25C48 does not execute replay, dry-run, condition changes, source changes, source recovery, live paths, AI API calls, Discord notifications, MT5 orders, live hooks, or final signals.

NO_SIGNAL Discord notification remains disabled.
