...
2. validation mode: a filled `gold_v2_24e_artifact_list_input.csv` is present in the 24E output folder before rerun; 24E validates required artifact/path/hash fields and can allow 24F only if all required rows are filled and no forbidden approval/execution fields are true.
...
| artifact list intake result | `gold_v2_24e_artifact_list_intake_result.csv` | one row per template/input row |
...
| required next gates | `gold_v2_24e_required_next_gates.csv` | allows only wait-for-artifact-list in template mode or 24F after valid input |
...
24E does not verify actual file content beyond intake structure; it validates the supplied artifact list only. Future review must verify artifact existence/hash separately.
...
Blank artifact input is not a STOP in template mode. It is a wait condition and must not allow 24F.
...
- source identity finalization
- artifact hash verification beyond intake presence/format
- live evaluator
- live hook
- final signal
- Discord notification
