# GOLD V2 20L value capture draft reconciliation specification

Date: 2026-06-06
Repo: `knitanr-a11y/xauusd-signal-lab`
Step: `20L_TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_RECONCILIATION_AUDIT_ONLY`
Mode: audit-only

## Purpose

20L reconciles the 20K value capture draft content-audit package.

20L is reconciliation-only. It does not collect a decision value, infer a decision value, approve anything, execute source recovery, finalize source identity, change signal rules, enable live/final paths, send Discord, place MT5 orders, or call AI APIs.

## Required upstream status

20K summary status must be:

`TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_CONTENT_AUDIT_PASSED_AUDIT_ONLY_SOURCE_RECOVERY_STILL_BLOCKED`

20K must have STOP rows 0 and `decision_value=UNSET`.

## Inputs

20K folder:

`FX_OUTPUTS/gold_v2_20k_tier2_source_identity_human_decision_value_capture_draft_content_audit_only`

Required files:

- `gold_v2_20k_tier2_source_identity_human_decision_value_capture_draft_content_audit_summary.json`
- `gold_v2_20k_content_checks.csv`
- `gold_v2_20k_allowed_value_audit.csv`
- `gold_v2_20k_required_field_audit.csv`
- `gold_v2_20k_required_next_gates.csv`
- `gold_v2_20k_safety_matrix.csv`
- `GOLD_V2_20K_TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_CONTENT_AUDIT_ONLY_REPORT.md`

Backup manifest:

- `docs/gold_v2/GOLD_V2_20L_PRE_CHANGE_BACKUP_MANIFEST_20260606.md`

## Reconciliation checks

20L checks:

- all inputs exist
- 20K status passed
- 20K STOP rows 0
- 20K content_audit_passed true
- 20K and safety decision value remain `UNSET`
- content checks have STOP rows 0
- allowed value audit failed rows 0
- required field audit failed rows 0
- safety matrix has STOP rows 0
- forbidden next gates remain false
- restricted summary flags remain false

## Outputs

Output folder:

`FX_OUTPUTS/gold_v2_20l_tier2_source_identity_human_decision_value_capture_draft_reconciliation_audit_only`

Outputs:

- `GOLD_V2_20L_TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_RECONCILIATION_AUDIT_ONLY_REPORT.md`
- `gold_v2_20l_tier2_source_identity_human_decision_value_capture_draft_reconciliation_summary.json`
- `gold_v2_20l_input_audit.csv`
- `gold_v2_20l_reconciliation_checks.csv`
- `gold_v2_20l_stage_status_audit.csv`
- `gold_v2_20l_required_next_gates.csv`
- `gold_v2_20l_safety_matrix.csv`

## Success status

`TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_RECONCILIATION_PASSED_AUDIT_ONLY_SOURCE_RECOVERY_STILL_BLOCKED`

## Next gate

Only after success:

`20M_TIER2_SOURCE_IDENTITY_HUMAN_DECISION_VALUE_CAPTURE_DRAFT_FINAL_AUDIT_ONLY`

Actual decision collection, source recovery, finalization, live, final signal, Discord, MT5, AI API, and live hook remain blocked.

## BAT

```bat
@echo off
cd /d "%~dp0\..\..\.."
python scripts\gold_v2_runtime\audit_gold_v2_20l_value_capture_draft_recon.py
pause
```
